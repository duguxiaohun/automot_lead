#!/usr/bin/env bash
# SFT base simple 训练 launcher：HIGHWAY/NON_HIGHWAY x RE/UE 四格随机均衡 + true torch DDP。
#
# 从 AutoMoT/ 目录运行：
...
TRAIN_INDEX="${TRAIN_INDEX:-checkpoints/sft_base_simple_data/train_sequence_index.jsonl}"
VAL_INDEX="${VAL_INDEX:-checkpoints/sft_base_simple_data/val_sequence_index.jsonl}"
MODEL_DIR="${MODEL_DIR:-checkpoints/Qwen3-VL-4B-Instruct}"
OUTPUT_DIR="${OUTPUT_DIR:-checkpoints/sft_base_simple_runs}"
...
case "${MODE}" in
  ...
  ddp)
    ...
    # sft_base_simple 使用普通 DDP + teacher-forced CE；这里仅负责进程数和 master 端口。
...
COMMON_ARGS=(
  ...
  --closed-loop-probe-fourbin-cases "${CLOSED_LOOP_PROBE_FOURBIN_CASES:-64}"
  --closed-loop-probe-gpu-ids "${CLOSED_LOOP_PROBE_GPU_IDS:-}"
  --max-steps "${MAX_STEPS:-0}"
  --frames-per-sync "${FRAMES_PER_SYNC:-64}"
  --fourbin-routes-per-batch "${FOURBIN_ROUTES_PER_BATCH:-16}"
  --highway-route-sample-target "${HIGHWAY_ROUTE_SAMPLE_TARGET:-0.5}"
  ...
  --road-loss-balance-mode "${ROAD_LOSS_BALANCE_MODE:-none}"
  ...
  --joint-balance-repeat-mode "${JOINT_BALANCE_REPEAT_MODE:-none}"
  --joint-balance-repeat-max "${JOINT_BALANCE_REPEAT_MAX:-8}"
  --joint-balance-repeat-combine "${JOINT_BALANCE_REPEAT_COMBINE:-add}"
  --joint-target-balance-mode "${JOINT_TARGET_BALANCE_MODE:-exact}"
  ...
  --memory-early-ue-frames "${MEMORY_EARLY_UE_FRAMES:-4}"
  --memory-early-ue-wrong-scale "${MEMORY_EARLY_UE_WRONG_SCALE:-1.75}"
  --memory-early-ue-unknown-scale "${MEMORY_EARLY_UE_UNKNOWN_SCALE:-1.35}"
  --memory-early-ue-dropout-scale "${MEMORY_EARLY_UE_DROPOUT_SCALE:-1.50}"
  --memory-early-ue-resample-prob "${MEMORY_EARLY_UE_RESAMPLE_PROB:-0.70}"
  --seed "${SEED:-20260711}"
  "${EXTRA_ARGS[@]}"
)
...
if [[ "${NPROC}" -gt 1 ]]; then
  torchrun --nproc_per_node="${NPROC}" \
    ...
    qwen3vl_local/sft_base_simple/train.py "${COMMON_ARGS[@]}"
else
  python qwen3vl_local/sft_base_simple/train.py "${COMMON_ARGS[@]}"
fi

echo "[done] adapter under ${OUTPUT_DIR}"
# --task 是必填项（road / event / full）；输出路径不传时会自动写到 adapter run 目录下。
echo "[hint] eval: GPU_IDS=0 python qwen3vl_local/sft_base_simple/eval.py --adapter-dir ${OUTPUT_DIR}/final --task full"
