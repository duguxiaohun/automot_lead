"""SFT base simple 的高速/非高速 + RE/UE 标签协议。

本文件保留 sft_base_simple 的 RS/EVENT 解析工具作为数据来源，但当前路线最终只监督
两个二分类值：
...
# 在 sft_base_simple 与 sft_v5 中候选集合和展示顺序一致，便于做逐样本直接对照；
...
def scenario_event_candidates_from_result(result: Mapping[str, Any]) -> List[str]:
    ...
    如果旧文件缺少该字段，就返回全量 EVENT_ORDER 作为审计兜底。sft_base_simple 当前
...
def q2_raw_candidates_for_frame(
    ...
    *,
    ...
    sft_base_simple 当前协议使用 RS 条件全集出题：allowed_events 只保留为 GT 解析和审计
    字段，不参与候选构造。
    """
