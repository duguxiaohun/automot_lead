...
from qwen3vl_local.sft_base_simple.train import RouteSequenceDataset, _build_inputs, _load_images, _messages  # noqa: E402
...
def setup_distributed() -> tuple[int, int, int]:
    ...
    if world_size > 1:
        if not torch.cuda.is_available():
            raise RuntimeError("sft_base_simple multi-GPU eval requires CUDA.")
...
def _validate_adapter_config(adapter_dir: pathlib.Path, model_dir: pathlib.Path, *, prompt_memory_mode: str = "memory") -> Dict[str, Any]:
    ...
    cfg_path = pathlib.Path(adapter_dir) / "sft_base_simple_adapter_config.json"
    if not cfg_path.exists():
        raise FileNotFoundError(f"sft_base_simple adapter config not found: {cfg_path}")
    with open(cfg_path, "r", encoding="utf-8") as f:
        cfg = json.load(f)
    if cfg.get("route") != "sft_base_simple_highway_reue_fourbin_random":
...
def load_eval_bundle(
    ...
    *,
    ...
    if adapter_dir is not None:
        ...
        print(f"[adapter] validated sft_base_simple adapter scope={cfg.get('lora_vision_scope')}")
...
def _select_full_route_cases(ds: RouteSequenceDataset, args: argparse.Namespace) -> List[EvalCase]:
    ...
    if str(args.full_balance_mode) == "joint":
        groups: Dict[str, List[Tuple[int, int]]] = {f"{road}:{event}": [] for road in ROAD_LABELS for event in EVENT_FAMILY_LABELS}
        route_items = list(enumerate(ds.rows))
        if int(args.sample_routes) > 0:
            rng = random.Random(int(args.seed))
            route_items = rng.sample(route_items, min(int(args.sample_routes), len(route_items)))
        elif int(args.max_routes) > 0:
            route_items = route_items[: int(args.max_routes)]
        for route_index, route in route_items:
            for frame_index, frame in enumerate(route.frames):
                ...
                key = f"{road}:{event}"
                groups.setdefault(key, []).append((route_index, frame_index))
        ...
        rng = random.Random(int(args.seed))
        selected_by_route: Dict[int, set[int]] = {}
        for key in sorted(groups):
            items = list(groups[key])
            rng.shuffle(items)
            for route_index, frame_index in items[: min(per_bin, len(items))]:
                selected_by_route.setdefault(route_index, set()).add(int(frame_index))
        cases: List[EvalCase] = []
        for route_index, selected in selected_by_route.items():
            route = ds.rows[route_index]
            ...
            cases.append(
                EvalCase(
                    ...
                    frames=list(route.frames[: max_index + 1]),
                    start_index=0,
                    score_frame_indices=set(selected),
                )
            )
        rng.shuffle(cases)
        return cases
...
def _evaluate_case(bundle: EvalBundle, case: EvalCase, args: argparse.Namespace, counters: Dict[str, int], jsonl_fp: Optional[TextIO]) -> None:
    ...
    score_indices = set(case.score_frame_indices) if case.score_frame_indices is not None else None
    counters["cases"] += 1
    ...
    for prev, cur in zip(rollout_records, rollout_records[1:]):
        counters["change_pairs"] += 1
...
def _default_eval_output_root(adapter_dir: Optional[str]) -> pathlib.Path:
    ...
    return pathlib.Path("checkpoints/sft_base_simple_runs/latest")
...
def _apply_eval_defaults(args: argparse.Namespace) -> None:
    ...
    if str(args.full_balance_mode) == "joint" and str(args.initial_memory_noise) == "none":
        raise ValueError("--initial-memory-noise none is not allowed with joint balanced eval; it leaks GT memory into sampled frames.")


def parse_args() -> argparse.Namespace:
    """解析 CLI 参数。"""

    p = argparse.ArgumentParser(description="Evaluate SFT base simple highway/RE-UE adapter")
    p.add_argument("--index", type=str, default="checkpoints/sft_base_simple_data/val_sequence_index.jsonl")
    ...
    p.add_argument("--eval-mode", choices=["full_route"], default="full_route")
...
def _write_html_report(path: pathlib.Path, metrics: Dict[str, Any], args: argparse.Namespace) -> None:
    ...
    title = f"SFT Base Simple Eval Report - {metrics.get('eval_mode', args.eval_mode)}"
    html = f"""<!doctype html>
<html lang="zh-CN">
<head>
  <meta charset="utf-8">
...
<body>
  <header>
    <h1>SFT Base Simple Eval Report</h1>
