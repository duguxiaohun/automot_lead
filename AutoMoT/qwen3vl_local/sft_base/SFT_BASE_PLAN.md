...
- `audit_rs_event_cooccurrence.py` 用来按 build_dataset 同款异常/缺失 route 过滤后统计 RS x UE 共现，检查静态表缺口和 R3 是否真是纯 RE；`audit_eval_candidate_drift.py` 用来扫 val index，量化 `pred_rs == gt_rs` 时 eval 静态候选相对训练候选 `event_candidates_ordered` 的 set/order 偏差，并分开输出 set mismatch、scoreable unreachable 与 dataset candidate mismatch examples；当前静态协议下 set/order mismatch 与 `unreachable_when_pred_eq_gt_scoreable_rate` 都应接近 0，dataset mismatch 作为上游数据缺陷单独报告。
