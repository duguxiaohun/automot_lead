...
- Q1 只判 `RS`，Q2 在串行上下文中判 `EVENT`；`ABNORMAL` 已删除，UE/regular 只从 Q2 EVENT 折叠。regular 不再合并成 `RE`，而是展开为 R-E1..R-E5 五个语义 token。
...
- 针对 checkpoint 消融暴露的视觉零增益、`ue_acc=0` 和 memory-copy shortcut，训练默认同时使用首帧 UNKNOWN、强 memory wrong/UNKNOWN 扰动、memory dropout、转折邻域重复、Q2 UE 加权和 regular 硬负样本。UE loss 按 RS 条件 UE 率 inverse-sqrt 缩放，regular loss 也按 R-E 子类频次 inverse-sqrt 缩放，避免模型只学到“R2 多猜 UE、R4/R5 多猜 regular”的条件先验。
...
- 评估额外返回 `joint_acc=P(RS 正确且 EVENT 正确)`、多数 regular 基线、净视觉增益、预测变化率、锁死 case 比例、UE 输出率、UE-vs-regular TP/FP/FN/TN 与 F1、单/多候选 × regular/UE 四格 EVENT 指标、按 RS 分组的 `ue_fp_on_multi_candidate_re_by_rs`、regular 内部混淆矩阵、dataset candidate mismatch 在全量/UE/regular 中的占比、相邻帧 RS change / regular->UE / UE->regular 的 TP/FP/FN/TN/F1/invalid、GT 稳定帧假转折率、转折命中时窗口左边界已在目标值的比例、RS/EVENT 转折方向混淆矩阵、RS/EVENT 混淆矩阵、GT EVENT 在学生 RS 候选下不可达比例、`*_hit_offset_avg`、`*_abs_hit_offset_avg`、`*_max_early_lead` 和 `*_max_late_lag`。GT EVENT 不在 dataset 自己候选表里的帧与训练侧 Q2 skip 对齐，不进入 EVENT 分母；GT EVENT 在 dataset 候选里但不在学生 RS 候选里，仍按模型错误计分。这样避免 `re_acc`、单候选送分题、数据缺陷或表面 transition hit rate 继续误导。
...
- 当前 `DATASET_VERSION=sft_base_rs_event_token_choice_re_expanded`，adapter config 的 `route=sft_base_token_choice`；旧的 A/B/C adapter 和旧 `RE -> REGULAR` adapter 都必须重训，会被 eval adapter config 校验拒绝。不要通过手动改 config 混跑旧权重。
...
- `EVENT_CANDIDATES_BY_RS` 按严格方案 A 维护 UE 组合：复用 build_dataset 的异常/缺失/失败 route 过滤后，只保留 `count >= 20` 且 `rs_frame_rate >= 0.1%` 的 RS×UE 组合。regular 当前仍是语义保守表，R3 已展开为 3 个 regular 候选；如果全量审计显示其它 RS×R-E 表外组合达到高频阈值，必须先看 scenario/route 归因，再决定扩表或修标注。
- `audit_rs_event_cooccurrence.py` 用来按 build_dataset 同款异常/缺失 route 过滤后统计 RS x UE、RS x R-E regular 共现、多 regular 标签比例、UE/regular 两侧 missing/low-rate/spurious 静态组合，并输出表外 regular 组合的 scenario/route top-k 归因；`audit_eval_candidate_drift.py` 用来扫 val index，量化 `pred_rs == gt_rs` 时 eval 静态候选相对训练候选 `event_candidates_ordered` 的 set/order 偏差，并分开输出 set mismatch、scoreable unreachable、dataset candidate mismatch 与 GT static candidate mismatch examples。候选表最终定版后，set/order mismatch 与 `unreachable_when_pred_eq_gt_scoreable_rate` 都应接近 0；定表前不要把 `--expect-mismatch-combinations` 当成通过门禁。
