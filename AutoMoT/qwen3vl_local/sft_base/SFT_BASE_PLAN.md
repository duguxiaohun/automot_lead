...
- 评估额外返回零视觉基线、净视觉增益、预测变化率、锁死 case 比例、UE 输出率、UE-vs-RE TP/FP/FN/TN 与 F1、相邻帧 RS change / RE->UE / UE->RE 的 TP/FP/FN/TN/F1/invalid、GT 稳定帧假转折率、转折命中时窗口左边界已在目标值的比例、RS/EVENT 转折方向混淆矩阵、RS/EVENT 混淆矩阵、GT EVENT 在学生 RS 候选下不可达比例、`*_hit_offset_avg`、`*_abs_hit_offset_avg`、`*_max_early_lead` 和 `*_max_late_lag`，避免 `re_acc` 或表面 transition hit rate 继续误导。
