...
- `EVENT_CANDIDATES_BY_RS` 按严格方案 A 维护 UE 组合：复用 build_dataset 的异常/缺失/失败 route 过滤后，只保留 `count >= 20` 且 `rs_frame_rate >= 0.1%` 的 RS×UE 组合；regular 子类直接保留各 RS 的 R-E 候选。当前 prompt 候选数为 R1=7、R2=5、R3=3、R4=5、R5=5，R3 不开放 UE 但不再是单候选。
- `audit_rs_event_cooccurrence.py` 用来按 build_dataset 同款异常/缺失 route 过滤后统计 RS x UE、RS x R-E regular 共现、多 regular 标签比例、missing/low-rate/spurious 静态组合；`audit_eval_candidate_drift.py` 用来扫 val index，量化 `pred_rs == gt_rs` 时 eval 静态候选相对训练候选 `event_candidates_ordered` 的 set/order 偏差，并分开输出 set mismatch、scoreable unreachable、dataset candidate mismatch 与 GT static candidate mismatch examples；当前静态协议下 set/order mismatch 与 `unreachable_when_pred_eq_gt_scoreable_rate` 都应接近 0，`--expect-mismatch-combinations` 只断言 GT static mismatch 的组合类型落在已知低频拒绝集合内，不按全量 163 帧硬套 val split。
