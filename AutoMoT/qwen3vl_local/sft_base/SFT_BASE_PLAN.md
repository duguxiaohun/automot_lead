...
- `eval.py` 的主 regular 零信息基线固定使用全量映射后最高频策略：R1/R2/R3 永远答 `LANE_FOLLOWING`，R4 永远答 `SIGNAL_COMPLIANCE`，R5 永远答 `PRIORITY_NEGOTIATION`；全量参考值约 76.85%。当前评估子集上事后挑多数类的 oracle baseline 只保留为审计字段，不能用于 `event_visual_gain_over_regular_baseline`。
