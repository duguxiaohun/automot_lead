...
- 评估额外返回零视觉基线、净视觉增益、预测变化率、锁死 case 比例、UE 输出率、UE-vs-RE TP/FP/FN/TN 与 F1、排除单候选送分题后的 `*_multi_candidate` EVENT 指标、dataset candidate mismatch 在全量/UE/RE 中的占比、相邻帧 RS change / RE->UE / UE->RE 的 TP/FP/FN/TN/F1/invalid、GT 稳定帧假转折率、转折命中时窗口左边界已在目标值的比例、RS/EVENT 转折方向混淆矩阵、RS/EVENT 混淆矩阵、GT EVENT 在学生 RS 候选下不可达比例、`*_hit_offset_avg`、`*_abs_hit_offset_avg`、`*_max_early_lead` 和 `*_max_late_lag`。GT EVENT 不在 dataset 自己候选表里的帧与训练侧 Q2 skip 对齐，不进入 EVENT 分母；GT EVENT 在 dataset 候选里但不在学生 RS 候选里，仍按模型错误计分。这样避免 `re_acc`、单候选送分题、数据缺陷或表面 transition hit rate 继续误导。
...
- `eval_candidates.py` 只要存在逐帧 allowed_events，就把 regular 与 UE 全部作为帧级事实保留并折叠 regular 为 `RE`；只有 allowed_events 缺失时才回落到 `scenario_event_candidates ∩ EVENT_CANDIDATES_BY_RS[pred_rs]`。当 eval 算出的候选集合与 dataset `event_candidates_ordered` 相同时，直接复用 dataset 顺序，避免 eval seed 造成 order drift。
- `audit_eval_candidate_drift.py` 用来扫 val index，量化 `pred_rs == gt_rs` 时 eval 候选过滤相对训练候选 `event_candidates_ordered` 的 set/order 偏差，并分开输出 set mismatch、scoreable unreachable 与 dataset candidate mismatch examples；修候选逻辑后 set/order mismatch 与 `unreachable_when_pred_eq_gt_scoreable_rate` 都应接近 0，dataset mismatch 作为上游数据缺陷单独报告。
