...
[VISUAL_CHECK_ORDER]
Classify the newest frame using the {history}. This is one road-structure decision written as four YES/NO lines: first look for a locally governing signal (RS4), then a local no-signal/priority junction (RS5), then an undivided opposing-direction constraint (RS2), then the ordinary surface-road fallback (RS1). Use older frames only to confirm a cue that is visible in the newest moment; never invent a hidden junction or road topology. Trace the ego vehicle's usable corridor and lane boundaries, then scan the complete left/front/right scene for a side road, cross street, stop/yield sign, signal mast or overhead arm, stop line, crosswalk, median, oncoming lane, and lane split. Do not make all four NO merely because rain, fog, darkness, glare, a vehicle, or a small cue makes the scene difficult: if the newest frame still visibly shows an ordinary surface-road corridor and no positive highway/ramp evidence, use RS1 as the fallback.{endpoint_notice}
[/VISUAL_CHECK_ORDER]

[DECISION_RULES]
RS1:
Ask: "Is the ego currently on an ordinary same-direction drivable surface road, outside a controlling junction, opposing-lane-sharing constraint, and highway/ramp decision structure?"
YES when the usable path is an ordinary lane or ordinary same-direction roadway: lane markings or road edges continue ahead, ordinary following/lane keeping/ordinary lane change is the main rule, and there is no positive local signal, priority-junction, or opposing-lane constraint. This includes local, urban, suburban, rural, parking-side, roundabout, and ordinary surface-road segments. It is also the fallback for a visible ordinary surface corridor in rain, fog, darkness, or traffic when the other three structures cannot be established from the pixels.
...
RS2:
Ask: "Does an undivided opposing direction define or constrain the ego's usable corridor, so the ego must reason about the oncoming lane?"
YES needs visible road-layout evidence, not a scenario name or a nearby parked car. Strong evidence is a narrow two-way corridor with a centre line and no median, especially when the front/lamps of an oncoming vehicle are visible across that line; or a fixed obstruction/parked vehicles/door leaving only a passage that requires entering or yielding to the opposite lane. The oncoming vehicle need not already block the ego in this exact frame: a clearly narrow, unseparated two-way corridor with directly relevant opposing traffic is enough. Four nominal lanes can still be RS2 when parked vehicles or obstacles remove the side lanes so that the remaining usable space is effectively one lane each way.
...
YES when the current visible junction has signal heads, masts, or overhead signal arms that govern the ego approach or intersecting approaches, plus at least one local junction cue such as a cross street, T-junction, turning pocket, stop line, crosswalk, median opening, or crossing traffic. Actively inspect the upper front view and both side views: in rain, glare, or a wide junction the physical signal head/arm can be small, high, partly masked, or on the far side, while its pole/arm remains visible. A failed, dark, flashing, or contradictory signal system is still RS4 when visible traffic-signal hardware is the rule source at this junction; Phase1 handles whether it is abnormal.
...
Usually exactly one of RS1/RS2/RS4/RS5 is YES. Do not decide the four lines as unrelated questions. All four NO requires positive visible highway/ramp/merge/exit evidence, such as a controlled high-speed mainline, ramp split, gore, or grade-separated connector. It is not the answer merely because an ordinary surface road is dark, foggy, empty, straight, rainy, or partly occluded. Do not invent an RS4/RS5/RS2 YES when its defining structure is absent from the newest visible scene.
[/DECISION_RULES]
""".strip()
    if audit:
        output = """
