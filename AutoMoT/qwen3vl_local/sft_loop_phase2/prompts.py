...
[VISUAL_CHECK_ORDER]
Classify the newest frame using the {history}. Trace the ego vehicle's usable corridor and lane boundaries, then scan the complete left/front/right scene for a side road, cross street, stop/yield sign, signal mast or overhead arm, stop line, crosswalk, median, oncoming lane, and lane split. Before calling an ordinary road RS1, actively check for a limited-access highway/ramp: several same-direction high-speed lanes with continuous guardrails/barriers, a merge/split/gore/exit, or a grade-separated connector are positive highway evidence and make all four answers NO. Then answer each question from what is visibly present now. Use older frames only to confirm a cue that is visible in the newest moment; never invent a hidden junction or road topology.{endpoint_notice}
[/VISUAL_CHECK_ORDER]
