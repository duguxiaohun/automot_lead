#!/usr/bin/env python3
"""评估 sft_loop_phase2 的 base Qwen 或 LoRA adapter。
...
def setup_distributed() -> Tuple[int, int, int]:
    ...
    if world_size > 1:
        if not torch.cuda.is_available():
            raise RuntimeError("sft_loop_phase2 multi-GPU eval requires CUDA.")
...
def _answer_topology(values: Mapping[str, Optional[str]]) -> str:
    ...
    if any(value not in ("YES", "NO") for value in normalized.values()):
        return "INVALID"
    positive = [key for key in ANSWER_KEYS if normalized[key] == "YES"]
    if not positive:
        return "ALL_NO"
    if len(positive) == 1:
        return positive[0]
...
def _topology_diagnostics(counter: Counter[str], total: int) -> Dict[str, Any]:
    ...
    pred_counts = {
        name: int(counter.get(f"pred/{name}", 0))
        for name in (*ANSWER_KEYS, "ALL_NO", "INVALID")
    }
    multi_counts = {
        name.removeprefix("pred/"): int(value)
        for name, value in sorted(counter.items())
        if name.startswith("pred/MULTI:")
    }
...
def _balanced_cases(rows: Sequence[FrameRow], *, cases_per_bin: int, seed: int) -> List[Tuple[FrameRow, str]]:
    ...
    if missing:
        raise ValueError(
            "cannot build exact 1:1 Phase2 evaluation cases: required focus bins are empty; "
...
def _validate_phase2_adapter(adapter_dir: pathlib.Path, model_dir: pathlib.Path) -> Dict[str, Any]:
    """读取 Phase2 adapter 自描述配置。"""

    cfg_path = adapter_dir / "sft_loop_phase2_adapter_config.json"
    if not cfg_path.exists():
        raise FileNotFoundError(f"missing phase2 adapter config: {cfg_path}")
    cfg = json.loads(cfg_path.read_text(encoding="utf-8"))
    if cfg.get("route") != "sft_loop_phase2_road_structure_four_binary":
...
def _resolve_history_rgb_mode(
    ...
    if requested_mode is not None:
        raise ValueError(
            ...
            "persisted history_rgb_mode from sft_loop_phase2_adapter_config.json."
        )
...
def parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(description="Evaluate base Qwen or Phase2 LoRA on balanced four-question cases")
    ...
    if not args.output_dir:
        name = "lora_rs_four_binary_final" if args.adapter_dir else "base_rs_four_binary_final"
        name += f"_{history_rgb_mode_tag(history_rgb_mode)}"
        if args.audit_prompt:
            name += "_audit"
        args.output_dir = str(_AUTOMOT_ROOT / "checkpoints/sft_loop_phase2_eval" / name)
    return args
