...
R3 不在本轮单独提问，因为 Phase1 已经负责 `HIGHWAY`。R3 高速主路、匝道、合流、分流或驶出帧仍会保留为
四问各自的 `NO` 监督样本，不会被丢弃或改标为 R1；模型不能根据“四项都为 NO”反推高速，而必须从 RGB 中自行
识别受控快速路、匝道或分合流证据。
...
`PHASE2_BASE_ERROR_RGB_AUDIT_20260813.md` 记录了 base Qwen production 错例的实际四帧 RGB
抽查、原始 LEAD RGB 路径与已有全帧审计的交叉核对。四问是完全独立的视觉判断：提示词不包含 RS1 强兜底、
互斥、优先级或答案模式反推。模型要分别看每一题的 RGB 证据，自行决定哪些为 YES、哪些为 NO。关键视觉定义是：

```text
高速/匝道：多条同向高速车道、连续隔离、导流、分合流或立交连接
RS4：可见灯具控制；RS5：可见无灯优先/STOP/YIELD 路口
RS2：未分隔且直接相关的对向通行约束；RS1：普通同向道路
```

多个 YES 或四个 NO 都是允许的模型原始输出，不会被代码改写。RS2 的窄正例是有中心线、相邻对向车/车灯且
两侧停放车辆或固定障碍压缩有效通行空间的窄双向走廊；即使 ego 本帧尚能沿本车道前进，仍可独立答 `RS2=YES`。
production 提示词还会在输出前为每题分别列出应扫描的局部视觉锚点；这是不输出的视觉注意提示，不是 thinking
输出，也不会把一题结果传给另一题。

这次只修改运行时提示词，不改 RS 原标签，也不要求重新构建索引。
...
`answer_pattern_diagnostics` 会记录四项全 `NO`、多个 `YES` 等原始输出模式，仅用于定位输出偏置；它不会约束
生成、解析、loss 或评分，也不能用 audit 分数代替 production 指标。
...
正式训练默认写 TensorBoard。另开一个终端，在 `AutoMoT/` 目录运行：
