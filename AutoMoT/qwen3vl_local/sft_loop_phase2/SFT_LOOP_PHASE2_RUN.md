...
抽查、原始 LEAD RGB 路径与已有全帧审计的交叉核对。提示词不用“RS1 强兜底”的串行分类：该写法曾让
base Qwen 在清晰的高速和信号路口也机械输出 RS1；也不允许“看不清其他结构”直接退化为四项 NO。当前
使用独立视觉判别：只有有正向受控快速路/匝道证据时四项才都为 NO。关键判别关系是：
...
普通地面道路通常四项中恰有一项为 `YES`；雾、雨、黑暗、直道、空路、远处灯光或不确定性都不是四项全
`NO` 的理由。RS2 的窄正例是有中心线、相邻对向车/车灯且两侧停放车辆或固定障碍压缩有效通行空间的窄
双向走廊；即使 ego 本帧尚能沿本车道前进，仍应答 `RS2=YES, RS1=NO`。
