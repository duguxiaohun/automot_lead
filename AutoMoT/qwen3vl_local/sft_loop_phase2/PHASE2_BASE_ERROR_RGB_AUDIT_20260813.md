...
`0.0000` 升至 `0.4902`。这证明 RS2 的可见窄双向走廊规则有效；但仍有 154 条预测为全 NO，实际
只有 16 条与 GT 的 R3 全 NO 一致，且有 26 条预测给出多个 YES。故下一步只加入最终输出一致性检查，
不再扩大各 RS 的视觉定义。
...
后续 eval 会自动写出 `topology_diagnostics`：GT/预测的一热结构分布、正确/错误全 NO 数与多 YES
组合，便于把真正的 prompt 收缩与上述视觉标签时间边界分开统计。
