...
最近的 base 结果显示：production 的 RS2 很低，但要求写可见证据的 audit 能重新识别部分中心线、相邻
迎面车和窄走廊。这不意味着 production 要输出思考或证据，也不意味着应加入跨题规则；因此提示词只新增
四个并列的、无输出的 question-local visual scan，分别把 audit 中实际有效的观察锚点放回对应问题。扫描
结果不能用于改变其他题答案，训练目标和四题逐项 loss 保持不变。

## 最新独立四问 production 复核：`20260814_133453`

本节逐帧读取该次 production 的 `error_cases/*/case.json` 所列四张原始输入 RGB，而不是仅依据
指标或场景名。该次使用的是独立四问提示词；下列观察也用于决定是否应继续改提示词。

| 主任务 | case 与模型错误 | 四帧实际可见事实 | 审计判断与处理 |
| --- | --- | --- | --- |
| RS2 FN | `ConstructionObstacleTwoWays/Town13/...003019.../f46`，GT `RS2=YES`，模型答 `NO` | 雨雾中的连续城市道路可见黄色中心线；中心线另一侧连续出现正面车灯/车辆，右侧贴近车道有停放红车，四帧均保留未隔离双向走廊。 | 标签有直接 RGB 支持。提示词可要求 RS2 单独扫描中心线、相邻迎面车辆/车灯和压缩走廊；不能要求已经借入对向车道。 |
| RS2 FP | `ConstructionObstacle/Town13/...1043.../f78`，GT `RS2=NO`，模型答 `YES` | 同样是雨雾城市道路、双黄线和中心线左侧的迎面车；路面相对宽、右侧可见连续车道和路缘，但与上一行的差异并不足以从单个四帧窗口稳定判别。 | 这是标签与可见 R2 线索存在张力的风险例，不能额外发明“宽道路必定 RS1”规则，否则会伤害上面的 R2 正例。 |
| RS1 FN | `ConstructionObstacle/Town12/...1256.../f7`，GT `RS1=YES`，模型答 `NO` | 四帧极暗，只能模糊看到前车尾灯、右侧边界和远处灯光；看不清可靠的同向道路结构、路口或对向关系。 | 当前 RGB 不足以支持把它教学为 RS1 正例，记录为低照度/时序视觉标签风险；不以该 case 扩写 RS1 兜底。 |
| RS4 FN | `VehicleTurningRoute/Town05/...Scenario4_2.../f34`，GT `RS4=YES`，模型答 `NO` 并答 `RS1=YES` | 四帧均可见宽斑马线、路口开口、左上横跨道路的多枚物理信号头，右侧也有信号杆/灯头；这些都在当前局部，而非远处像素。 | 标签有强 RGB 支持。保留 RS4 的上方前视与两侧信号硬件单独扫描，避免模型只注意车道/前车。 |
| RS5 FN | `OppositeVehicleTakingPriority/Town12/...1627.../f15`，GT `RS5=YES`，模型答 `NO` | 连续四帧是住宅道路、双黄线、路缘与前车；最近画面内未见横向道路、T 口、STOP/YIELD、路口盒或明确优先冲突。 | 当前 RGB 无法确认 RS5，属于时间边界风险；不能因场景名含 priority 或因前方车辆而强推 RS5。 |
| RS5 FP | `SignalizedJunctionLeftTurnEnterFlow/Town05/...002256.../f26`，GT `RS4=YES, RS5=NO`，模型同时答 `RS4=YES, RS5=YES` | 雾中仍连续可见横向道路、斑马线、多根悬臂信号杆及亮起的信号头。 | RS4 有可见支持，RS5 的 NO 也有可见支持，因为本题自己的“working traffic signal”否定条件成立。RS5 局部扫描必须同时检查实体信号硬件，不能把任何交叉口泛化成 RS5。 |

由这一轮可验证的帧得出的代码结论只有一项：在不输出证据、不改变四个 YES/NO 独立性的前提下，加入
`QUESTION_LOCAL_VISUAL_SCANS`，让每题在回答前查看与自己定义对应的实体锚点。它不是答案模式约束、不是
隐藏标签，也不是把一题结果传给另一题。上表中无法由当前 RGB 支持的低照度/时间边界样本继续只作为
`visual_label_risk` 候选；本轮不改原始标签，也不因此重构数据集。

## 局部扫描回退复核：`20260814_151310`

上述局部扫描提示经新的 base production 验证后不能保留：四题 exact 从上一独立提示词的 `0.3164` 降为
`0.2949`，主任务 F1 为 RS1 `0.6265`、RS2 `0.0299`、RS4 `0.5778`、RS5 `0.7170`。RS1 的表面 F1
上升来自 64 个 RS1 负例中误报 50 个；RS2 的 64 个正例只命中 1 个。因此这不是可用于训练的改进。

本轮再逐帧查看新的 production 错例后，得到如下可操作结论：

| case | 四帧可见事实 | 提示词处理 |
| --- | --- | --- |
| `MergerIntoSlowTraffic/Town12/...4141.../f55`，GT 四项均 NO，模型答 `RS1=YES` | 夜间多条同向车道、双侧连续护栏/隔离与宽路肩全部清楚可见；没有普通城市路缘或局部路口。 | 删除会优先强调“连续普通同向车道”的 RS1 局部扫描。在 RS1 自身 NO 定义中仅补入这一可见高速走廊特征。 |
| `ConstructionObstacleTwoWays/Town13/...003019.../f46`，GT `RS2=YES`，模型答 `RS1=YES, RS2=NO` | 黄中心线、中心线另一侧相邻迎面车辆/车灯和路侧停放车在四帧中可见。audit 可写出这些证据并答 RS2 YES，production 仍漏检。 | 这说明 base 在短输出时不能稳定保留细粒度证据；不应通过让 RS1/RS2 相互排斥或要求答案模式来修复。保留既有 RS2 可见定义，等待独立 LoRA 监督。 |
| `VehicleTurningRoute/Town05/...Scenario4_2.../f34`，GT `RS4=YES`，模型答 `RS1=YES, RS4=NO` | 斑马线、横向路口、悬臂灯杆和多枚实体信号头均清楚可见。 | 局部扫描没有提高该类 RS4 漏检；维持物理灯具加本地路口的定义，后续以干净正例训练，不再加冗长提示。 |
| `AccidentTwoWays/Town13/...1554.../f90` 与 `VehicleTurningRoute/Town12/...1908.../f50` | 前者雨雾中对向关系无法可靠确认；后者极暗，只能看到前车和远处禁行标志，无法确认局部无信号路口。 | 继续作为 `visual_label_risk` 候选，不拿来强扩 RS2/RS5 定义。 |

因此代码回退 `QUESTION_LOCAL_VISUAL_SCANS`，不加入任何互斥、四选一、优先级、答案模式或“四个 NO 即高速”规则。
四题仍分别按本题的 RGB 定义输出 YES/NO，并按四个 value token 分别计算 loss。唯一保留的窄调整是 RS1 自身的
高速视觉否定条件，且只来自上表 `f55` 的双侧连续护栏、多同向车道和宽路肩证据。
