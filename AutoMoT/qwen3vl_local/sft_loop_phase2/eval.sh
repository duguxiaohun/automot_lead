#!/usr/bin/env bash
# Phase2 原版一键评测：base production/audit -> LoRA production/audit -> sampled error RGB -> <=30MB 审计包。
#
# 从 AutoMoT/ 目录运行：
#   ADAPTER_DIR=checkpoints/sft_loop_phase2_runs/run_rs_four_binary_format_supervised_4rgb/final bash qwen3vl_local/sft_loop_phase2/eval.sh
# 或：
#   bash qwen3vl_local/sft_loop_phase2/eval.sh checkpoints/sft_loop_phase2_runs/run_rs_four_binary_format_supervised_4rgb/final
...
PHASE_NAME="sft_loop_phase2"
EVAL_PY="qwen3vl_local/sft_loop_phase2/eval.py"
...
if [[ -z "${ADAPTER_INPUT}" ]]; then
  echo "Usage: ADAPTER_DIR=<lora-adapter-or-run-dir> bash qwen3vl_local/sft_loop_phase2/eval.sh" >&2
  exit 2
fi
...
for name in ("sft_loop_phase2_adapter_config.json", "sft_loop_phase1_adapter_config.json"):
    path = adapter / name
    if path.is_file():
...
def build_attempt(case_limit: int, max_side: int, quality: int) -> int:
    if bundle.exists():
...
for attempt in [(28, 768, 60), (18, 640, 55), (10, 560, 50), (6, 448, 45), (3, 384, 40)]:
