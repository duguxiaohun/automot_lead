#!/usr/bin/env python3
"""训练 sft_new_loop_phase2 的单轮直接 EVENT 二元问题 LoRA adapter。

训练目标是 UE1/UE3/UE5/UE6/INVALID YES/NO 语义 token，
并以低权重监督字段格式和 assistant 结束符。数据构建阶段保证四个 UE 正类
1:1:1:1，保留 valid RE/R3 highway hard negatives，并额外注入约 20% 跨问题域
invalid 样本。输入没有 synthetic ROAD_STRUCTURE user/assistant 前缀。
"""
...
def setup_distributed() -> Tuple[int, int, int]:
    ...
    if world_size > 1:
        if not torch.cuda.is_available():
            raise RuntimeError("sft_new_loop_phase2 DDP requires CUDA.")
...
def _write_run_metadata(
    ...
    *,
    ...
    payload = {
        ...
        "generation_eval_min_applicable_regular_exact": float(
            args.generation_eval_min_applicable_regular_exact
        ),
...
def _resolve_rgb_path(raw: str, data_root: pathlib.Path) -> str:
    """解析新相对路径，并兼容旧机器写出的 lead_data 绝对路径。"""
...
def _raw_focus_bin_counts(rows: Sequence[FrameRow]) -> Dict[str, int]:
    """统计 class、问题域、真实 RS、答案和 invalid 子类别。"""
...
def _iter_candidate_items(rows: Sequence[FrameRow], *, seed: int) -> Iterable[WorkItem]:
    """逐个生成直接 EVENT 候选；每帧只对应一个问题域 prompt。"""
...
def _balanced_work(
    ...
    *,
    ...
    target_per_bin>0 时按各 class 截取/循环；为 0 时以六个必需 class 的最小原始桶
    作为共同基数，而不是直接保留不均衡 raw rows。
    ...
    if missing:
        raise ValueError(
            "direct-event training balance requires UE1/UE3/UE5/UE6/RE/INVALID buckets; "
    ...
    if effective_target <= 0:
        effective_target = min(int(class_counts[key]) for key in required_classes)
    ...
    if not groups:
        return []
    rng = random.Random(
        f"{seed}:new_phase2_balance:{len(rows)}:{effective_target}:"
    ...
    for key in sorted(groups):
        ...
        target = _balance_target_for_key(
            key,
            target_per_bin=effective_target,
        ...
        if target <= 0:
            ...
            if highway_target > 0 and not highway:
                raise ValueError("RE balance requires R3/highway rows but none are available")
...
def _effective_class_target(rows: Sequence[FrameRow], requested: int) -> int:
    """返回六个必需 EVENT class 的共同基数，不读取旁路统计桶。"""
    ...
    if missing:
        raise ValueError(
            f"cannot infer six-class target; missing={missing} available={dict(sorted(class_counts.items()))}"
        )
    return min(int(class_counts[key]) for key in required_classes)
...
def _target_token_weights(
    ...
    *,
    ...
    的语法锚定，使用较低权重；它们绝不编码各输出题之间的任何逻辑关系。
    """
...
def _build_inputs(
    ...
    *,
    ...
    """构造模型输入，主监督答案值并低权重监督当前输出行格式与结束符。"""
...
def evaluate_loss(
    ...
    *,
    ...
    invalid_subgroup_ok: Counter[str] = Counter()
    invalid_subgroup_total: Counter[str] = Counter()
    for item in work:
        row = item.row
        spec = item.spec
    ...
    subgroup_payloads = [
        {
            "ok": dict(invalid_subgroup_ok),
            "total": dict(invalid_subgroup_total),
        }
    ]
    if int(world_size) > 1:
        subgroup_payloads = [None for _ in range(int(world_size))]  # type: ignore[list-item]
        dist.all_gather_object(
            subgroup_payloads,
            {"ok": dict(invalid_subgroup_ok), "total": dict(invalid_subgroup_total)},
        )
    merged_subgroup_ok: Counter[str] = Counter()
    merged_subgroup_total: Counter[str] = Counter()
    for payload in subgroup_payloads:
        merged_subgroup_ok.update(payload.get("ok", {}))
        merged_subgroup_total.update(payload.get("total", {}))
    for key, count in sorted(merged_subgroup_total.items()):
        metrics[f"invalid_subgroup/{key}_samples"] = float(count)
        metrics[f"invalid_subgroup/{key}_exact"] = float(merged_subgroup_ok.get(key, 0)) / max(
            1.0, float(count)
        )
    if was_training:
        bundle.model.train()
...
def evaluate_generation_probe(
    ...
    *,
    ...
    """在固定独立 val 样本上以真实 greedy generation 检查当前输出行格式。

    这是训练选 best adapter 的防退化闸门。它不改变答案、解析或各题独立性；只用
    ...
    for item in work:
        ...
        parsed = parse_event_output(raw, spec=spec)
...
def generation_checkpoint_score(
    ...
    *,
    ...
    min_ue6_target_recall: float = 0.0,
    min_invalid_exact: float = 0.0,
    min_applicable_regular_exact: float = 0.0,
    ...
    ue6_recall = float(metrics.get("slice/ue6_target_recall", metrics.get("slice/ue6_exact", 0.0)))
    invalid_exact = float(metrics.get("slice/invalid_exact", 0.0))
    applicable_regular_exact = float(metrics.get("slice/applicable_regular_exact", 0.0))
    exact = float(metrics.get("exact_accuracy", 0.0))
    pattern_exact = float(metrics.get("pattern/all_random_order_pattern_exact", 0.0))
    values_and_floors = (
        (ue3_recall, float(min_ue3_target_recall)),
        (ue6_recall, float(min_ue6_target_recall)),
        (invalid_exact, float(min_invalid_exact)),
        (applicable_regular_exact, float(min_applicable_regular_exact)),
    )
    passed = [value >= floor for value, floor in values_and_floors]
    ratios = [1.0 if floor <= 0.0 else value / floor for value, floor in values_and_floors]
    all_guards_ok = all(passed)
    if all_guards_ok:
        return (1.0, exact, min(ratios), pattern_exact, ue3_recall, ue6_recall)
    return (
        0.0,
        float(sum(passed)) / float(len(passed)),
        min(ratios),
        exact,
        pattern_exact,
        min(ue3_recall, ue6_recall),
    )
...
def generation_checkpoint_guards(
    ...
    *,
    min_ue3_target_recall: float,
    min_ue6_target_recall: float,
    min_invalid_exact: float,
    min_applicable_regular_exact: float,
) -> Dict[str, Any]:
    """返回 checkpoint 多指标门槛的逐项审计结果。"""

    values = {
        "ue3_target_recall": float(
            metrics.get("slice/ue3_target_recall", metrics.get("slice/ue3_exact", 0.0))
        ),
        "ue6_target_recall": float(
            metrics.get("slice/ue6_target_recall", metrics.get("slice/ue6_exact", 0.0))
        ),
        "invalid_exact": float(metrics.get("slice/invalid_exact", 0.0)),
        "applicable_regular_exact": float(metrics.get("slice/applicable_regular_exact", 0.0)),
    }
    floors = {
        "ue3_target_recall": float(min_ue3_target_recall),
        "ue6_target_recall": float(min_ue6_target_recall),
        "invalid_exact": float(min_invalid_exact),
        "applicable_regular_exact": float(min_applicable_regular_exact),
    }
...
def _save_adapter(bundle: Any, output_dir: pathlib.Path, args: argparse.Namespace, *, step: int, name: str = "final") -> pathlib.Path:
    ...
    cfg = {
        ...
        "data_root": str(args.data_root),
        "input_contract": "single image+text user turn; no synthetic ROAD_STRUCTURE or assistant prefix",
...
def train(args: argparse.Namespace) -> None:
    ...
    effective_focus_target_per_class = _effective_class_target(rows, int(args.focus_balance_count))
    # 第一个 epoch 的全局 work；后续 epoch 会以不同 seed 重建，富余桶不会固定重复
    # 同一小批样本，稀缺桶按需循环重采样，守住六个必需 EVENT class 的目标数。
    if rank == 0:
        print(
            f"[startup] building train work target_per_class={effective_focus_target_per_class} "
    ...
    if int(args.eval_steps) > 0 and int(args.eval_balance_count) > 0:
        try:
            ...
            raise RuntimeError(
                "periodic validation was requested but its split cannot satisfy the required six-class balance. "
    ...
    if rank == 0:
        ...
        if eval_work:
            val_balance = {
                ...
                "rank0_invalid_subgroups": invalid_subgroup_report(eval_work),
            }
            ...
            json.dumps(
                {
                    ...
                    "train": {
                        ...
                        "rank0_invalid_subgroups": invalid_subgroup_report(work),
                    ...
                    "generation_eval": {
                        ...
                        "min_ue6_target_recall": float(args.generation_eval_min_ue6_target_recall),
    ...
    best_generation_score = (-math.inf,) * 6
    best_fallback_score = (-math.inf,) * 6
            ...
            if eval_work and int(args.eval_steps) > 0 and global_step % int(args.eval_steps) == 0:
                ...
                if rank == 0:
                    ...
                    print(
                        ...
                        f"direct_event_exact={metrics.get('variant/all_random_order_exact', 0.0):.4f}"
                    )
                ...
                if run_generation_eval and rank == 0:
                    ...
                    print(
                        ...
                        f"direct_event_valid={generation_metrics.get('variant/all_random_order_valid', 0.0):.4f}"
                    )
...
def parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(description="Train sft_new_loop_phase2 single-turn direct-EVENT LoRA")
    ...
    p.add_argument(
        ...
        type=float,
        default=1.0,
        help="training-only multiplier for cross-domain invalid samples",
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        type=float,
        default=0.80,
        help="minimum UE6-positive recall for production-ready best_generation",
    )
    p.add_argument(
        "--generation-eval-min-invalid-exact",
        type=float,
        default=0.80,
        help="minimum INVALID class exact for production-ready best_generation",
    )
    p.add_argument(
        "--generation-eval-min-applicable-regular-exact",
        type=float,
        default=0.50,
        help="minimum applicable-local RE exact for production-ready best_generation",
    )
    ...
    for arg_name in (
        "generation_eval_min_ue3_target_recall",
        "generation_eval_min_ue6_target_recall",
        "generation_eval_min_invalid_exact",
        "generation_eval_min_applicable_regular_exact",
    ):
        if not 0.0 <= float(getattr(args, arg_name)) <= 1.0:
            raise ValueError(f"--{arg_name.replace('_', '-')} must be in [0, 1]")
    ...
    if not args.output_dir:
        args.output_dir = str(
            _AUTOMOT_ROOT
            / "checkpoints/sft_new_loop_phase2_runs"
            / f"run_direct_event_format_supervised_{history_rgb_mode_tag(args.history_rgb_mode)}"
        )
    return args
