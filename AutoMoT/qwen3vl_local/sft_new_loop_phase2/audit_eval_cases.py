#!/usr/bin/env python3
"""从 new Phase2 eval 产物中抽取少量错例并补齐真实 RGB。
...
DEFAULT_EVAL_DIR = _AUTOMOT_ROOT / "checkpoints/sft_new_loop_phase2_eval/base_direct_event_final_4rgb"
DEFAULT_OUTPUT_DIR = _AUTOMOT_ROOT / "checkpoints/sft_new_loop_phase2_audit_samples/base_direct_event_4rgb"
...
def _target_matches(payload: Mapping[str, Any]) -> List[str]:
    ...
    matched: List[str] = []
    if any(key in gt and parsed.get(key) not in ("YES", "NO") for key in tuple(EVENT_KEYS) + (INVALID_KEY,)):
    ...
    if gt.get(INVALID_KEY) == "YES" and any(parsed.get(key) == "YES" for key in EVENT_KEYS):
        matched.append("invalid_context_not_all_no")
    if _yes_count(parsed, EVENT_KEYS) > 1:
        matched.append("multi_ue_yes")
    return matched
...
def _render_note(row: Mapping[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            "- defining actor visible in newest frame: `YES / NO / AMBIGUOUS`",
            "- label visually supported: `YES / NO / AMBIGUOUS`",
...
def build_audit_samples(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    invalid_subgroup_counts: Dict[str, Counter[str]] = defaultdict(Counter)
    for row in manifest_rows:
...
def _render_summary(summary: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# new Phase2 Direct-Event Eval Audit Samples",
        "",
