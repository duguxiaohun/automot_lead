...
PROMPT_NAME = "sft_loop_phase2_rs_augmented_visual_v1"
...
def build_phase2_prompt(
    *,
    ...
    output_lines = "\n".join(
        (
            f"EVIDENCE_{q.output_key}: <short visible RGB observation>\n{q.output_key}: <YES or NO>"
            if audit
            else f"{q.output_key}: <YES or NO>"
        )
        for q in spec.questions
    )
