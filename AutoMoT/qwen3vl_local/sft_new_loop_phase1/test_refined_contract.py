...
class RefinedContractTest(unittest.TestCase):
    ...
    def test_pipeline_and_eval_own_rgb_mode_contract(self) -> None:
        ...
        self.assertIn('RUN_BALANCED_EVAL="${RUN_BALANCED_EVAL:-1}"', pipeline)
        final_eval_block = pipeline.split('if [[ "${RUN_EVAL_SH}" == "1" ]]', 1)[1]
        self.assertIn("bash qwen3vl_local/sft_new_loop_phase1/eval.sh", pipeline)
        self.assertIn("run_final_eval_bundle", final_eval_block)
        self.assertIn("best_generation_balanced", final_eval_block)
        self.assertIn("adapters_have_same_weights", pipeline)
        self.assertIn("eval_review_balanced", final_eval_block)
