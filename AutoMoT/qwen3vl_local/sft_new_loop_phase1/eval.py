#!/usr/bin/env python3
"""评估 sft_new_loop_phase1 的 base Qwen 或 LoRA adapter。

默认对八个主任务分别做 YES/NO 1:1 抽样，并保存 `cases.jsonl`、`metrics.json`、
...
def setup_distributed() -> Tuple[int, int, int]:
    ...
    if world_size > 1:
        if not torch.cuda.is_available():
            raise RuntimeError("sft_new_loop_phase1 multi-GPU eval requires CUDA.")
...
class WorkItem:
    """一条带 Phase2 augment spec 的评估 case。"""
...
def _variant_for_slot(slot: int) -> str:
    """按 phase2 eval 权重把 slot 映射到增强 variant。"""

    schedule: List[str] = []
    for name, weight in VARIANT_WEIGHTS.items():
        schedule.extend([str(name)] * max(0, int(weight)))
    if not schedule:
        raise ValueError("empty eval variant weights")
    return schedule[int(slot) % len(schedule)]


def _make_spec(row: FrameRow, focus: str, *, seed: int, slot: int) -> PromptSpec:
    """构造当前 eval case 的融合 prompt spec。"""

    variant = _variant_for_slot(slot)
    phase2_keys = ["RS1", "RS2", "RS4", "RS5"]
    phase2_focus = focus if focus in phase2_keys else phase2_keys[slot % len(phase2_keys)]
    group_ids = sorted(GROUP_DEFINITIONS.keys())
    return make_prompt_spec(
        variant=variant,
        answers=row.answers,
        seed_key=f"eval:{seed}:{row.scenario}:{row.route_id}:{row.frame_id}:{focus}:{slot}",
        focus=phase2_focus,
        subset_count=int(SUBSET_COUNTS[slot % len(SUBSET_COUNTS)]),
        group_id=group_ids[slot % len(group_ids)],
        detail_key=phase2_focus,
    )
...
def _validate_phase1_adapter(adapter_dir: pathlib.Path, model_dir: pathlib.Path) -> Dict[str, Any]:
    """读取 fused-loop adapter 自描述配置。"""

    cfg_path = adapter_dir / "sft_new_loop_phase1_adapter_config.json"
    if not cfg_path.exists():
        raise FileNotFoundError(f"missing sft_new_loop_phase1 adapter config: {cfg_path}")
    cfg = json.loads(cfg_path.read_text(encoding="utf-8"))
    if cfg.get("route") != "sft_new_loop_phase1_phase1_phase2_visible_facts":
...
def _resolve_history_rgb_mode(
    ...
    if requested_mode is not None:
        raise ValueError(
            ...
            "persisted history_rgb_mode from sft_new_loop_phase1_adapter_config.json."
        )
...
def load_eval_bundle(model_dir: pathlib.Path, adapter_dir: Optional[pathlib.Path], device: torch.device, *, merge_lora: bool) -> EvalBundle:
    """加载 base Qwen 和可选 fused-loop LoRA。"""
...
def evaluate(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    with case_path.open("w", encoding="utf-8") as f:
        try:
            for local_idx, item in enumerate(local_cases):
                ...
                main_gt_value = gt.get(main_output_key, "INVALID")
    ...
    metrics = {
        ...
        "sampling_contract": (
            "Eight independent task modules. In each module, that task's GT YES/NO cases are sampled 1:1; "
            "each prompt asks the four Phase1 questions plus the current Phase2 augment variant lines. "
            "Four modules are Phase1 and four are Phase2, so Phase1-focus and Phase2-focus cases are sampled 1:1."
        ),
...
def parse_args() -> argparse.Namespace:
    ...
    if not args.output_dir:
        name = "lora_combined_phase1_phase2" if args.adapter_dir else "base_combined_phase1_phase2"
        name += f"_{history_rgb_mode_tag(history_rgb_mode)}"
        if args.audit_prompt:
            name += "_audit"
        args.output_dir = str(_AUTOMOT_ROOT / "checkpoints/sft_new_loop_phase1_eval" / name)
    return args
