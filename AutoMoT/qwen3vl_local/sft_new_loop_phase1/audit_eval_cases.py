...
def _copy_case(
    ...
    *,
    ...
    row = {
        ...
        "audit_diagnostics": payload.get("audit_diagnostics"),
...
def _render_note(row: Mapping[str, Any]) -> str:
    return "\n".join(
        [
            ...
            f"- audit_diagnostics: `{json.dumps(row.get('audit_diagnostics'), ensure_ascii=False)}`",
            ...
            "## Manual RGB verdict",
            "",
            "Review every copied history frame before filling this block; do not infer from scenario/GT alone.",
            "",
            "- target_key: `TODO`",
            "- label_support: `TODO: strong / weak / unsupported / uncertain`",
            "- newest_frame_witness: `TODO`",
            "- older_frame_witness: `TODO`",
            "- temporal_relevance_at_newest: `TODO`",
            "- error_source: `TODO: model / label / visibility / temporal_boundary / parser`",
            "- recommended_action: `TODO: prompt / loss / data-risk / keep-as-is`",
            "- concise_rgb_reason: `TODO`",
            "",
        ]
    )
