...
每条样本 assistant 内的 ANALYSIS 段写入特殊占位 `__TEACHER_PENDING__`，
`dataset_version="pending"`。训练时由 `train.py` 在每个 train batch 内现场用
frozen base Qwen3-VL-Instruct（PEFT disable_adapter 上下文）即时生成 ANALYSIS
真值并替换占位；不会落盘 teacher cache，每次启动训练都重新现场跑。
...
4. 对每个采样帧拼 messages（system + user + assistant_pending），写 train / val jsonl。

注意：assistant 的 STATUS 段是当前帧 GT，user 的 MEMORY 块是上一帧 GT —— 防 leak。
teacher 推理用的 PRIVILEGED prompt 由 train.py / build_teacher.py 临时拼装，不落盘。
...
python qwen3vl_local/sft/build_dataset.py --dry-run --output-dir /tmp/sft_dry
```

输出文件：
- train.jsonl / val.jsonl：每行一个多模态 chat 样本，dataset_version=="pending"。
- stats.json：每个场景保留/推进样本数量。
- val.jsonl 按 run_id 切，避免同一 route 的相邻帧同时出现在 train/val。

重要边界：
- 本脚本只生成图片路径，不读取图片内容；真正图片读取发生在 train.py / eval.py。
...
# 把 AutoMoT 加入 sys.path，复用 prompt_pipeline 里的系统提示词、记忆结构和状态机。
...
# 上一帧距当前帧的间隔 K。LEAD 数据约 4 Hz，K=4 约等于 1 秒前。详见 PLAN §1。
DEFAULT_K_FRAMES = 4

# 保持类样本只避开 GT 转换帧前的缓冲帧，避免把临界前帧当成稳定保持状态。
DEFAULT_BOUNDARY_BUFFER = 2

# 每帧图像采样的步长和数量，需要与 runner 默认值（image_io.load_lead_rgb_clip）一致。
RGB_FRAME_STEP = 1
RGB_FRAME_COUNT = 4

# 训练时只接受这两个运行状态。其它状态（如 "Failed" / "Crashed"）的 GT 标注不可信。
ACCEPTED_RUN_STATUS = {"Completed", "Perfect"}

# 待填充的 ANALYSIS 占位。train.py 会在每个训练 batch 内现场运行冻结的 base Qwen，
# 用真实 ANALYSIS 文本替换该串。这里故意选用自然语言里不会出现、且 tokenizer
# 容易切开的哨兵字符串。
PLACEHOLDER_ANALYSIS_PENDING = "__TEACHER_PENDING__"

# teacher 在某条样本上失败时使用的兜底文本（train.py / build_teacher.py 共用）。
...
def collect_candidates(
    ...
    本任务的核心痛点是"模型太早推进"，所以 keep 样本只避开转换前 buffer；
...
def stratify_scenario(
    ...
    # ---- 为什么用 id() 集合，而不是 `s in chosen_keep` ----
    # SampleRecord 是 @dataclass，Python 会自动生成 __eq__：两个实例只要所有字段值
    # 都相等，就会被视为相等。这意味着：
    #   * `if s not in chosen_keep` 会按"值"判等，而不是按"是不是同一个对象"判等；
    #   * 如果某一天 collect_candidates() 为同一 (run_id, anchor) 产生两个独立的
    #     SampleRecord 实例（例如多次调用后合并，或新增字段后字段值恰好相同），
    #     值相等的 record 会被错误剔除，导致 remaining 漏掉真正"未被选中的对象"。
    # 这里 (run_id, anchor) 实际唯一，所以当前不会出错；但用对象 id 做集合判重
    # 不依赖 dataclass __eq__ 行为，未来加字段或改字段也更稳。
    # 另外一个好处：id() 集合查询是 O(1)，比 `in list` 的 O(n) 在补齐循环里更快。
    chosen_ids: set = set()
    if buckets:
...
def build_image_paths(
    ...
    # 与 qwen3vl_local.image_io.load_lead_rgb_clip 保持同一套容错逻辑。
    #
    # 为什么不能简单写死 f"{idx:04d}.jpg"：
    # - keyframes 的 frame 是从 0 开始的索引；
    # - 某些 LEAD route 的落盘文件可能从 0001.jpg 起步；
    # - runner 读取时已经有回退逻辑，会取 sorted(rgb/*.jpg)[idx]。
    #
    # 训练数据如果不采用同样回退逻辑，LoRA 训练可能会看到不存在的图片路径，
...
def build_messages(
    ...
    - assistant.content 是 GT 三段输出，ANALYSIS 段为 ``__TEACHER_PENDING__``
      占位，由 train.py / build_teacher.py 现场跑 frozen base Qwen 替换。

    user prompt 复用 qwen3vl_local.prompt_pipeline，保证训练时看到的 system/user
    指令和 standalone runner 尽量一致。
    """
    ...
    # 多模态 user content 用 <image> 占位符标注图片插入位置。每张图一个 <image>。
    # eval.py / probe.py 在复原 prompt 时会去掉这些占位符，再交给本项目的结构化
    # 图像消息路径；`<image>` 只服务于 jsonl schema 显式标注，不进入本地 engine。
    user_content = "".join("<image>" for _ in image_paths) + "\n" + user_text

    target_subgoal = _next_event_in_seq(scenario, sample.target_status)
    assistant_content = (
        f"ANALYSIS: {PLACEHOLDER_ANALYSIS_PENDING}\n"
    ...
    return {
        ...
        # train.py / build_teacher.py 拼 PRIVILEGED prompt 时直接读这块，
        # 不用再从 messages[2] 反向解析。
        "teacher_meta_input": {
...
def main():
    """构建 pending SFT 数据集：筛选 keyframes、采样 keep/advance，并写 train/val jsonl。"""
    ...
    # 800/0.35 是当前默认：全集约 14000 样本，推进类比例 35%。
    # 配合 train.sh 默认 num_epochs=2 / lr=3e-5，等效 batch=32 时总 step 约 900。
