...
#   bash qwen3vl_local/action_expert_ablation/bev_only/train.sh --event-balanced --rgb-frame-count 1
#   GPU_IDS=0,1,2,3 bash qwen3vl_local/action_expert_ablation/bev_only/train.sh --event-balanced --rgb-frame-count 1
# 单当前图＋high-level 动作 token（KEEP 不细分）：
#   bash qwen3vl_local/action_expert_ablation/bev_only/train.sh --event-balanced --rgb-frame-count 1 --high-level-action-token
#   GPU_IDS=0,1,2,3 bash qwen3vl_local/action_expert_ablation/bev_only/train.sh --event-balanced --rgb-frame-count 1 --high-level-action-token
# 环境变量等价写法；显式 CLI 优先：
#   RGB_FRAME_COUNT=1 HIGH_LEVEL_ACTION_TOKEN=1 bash qwen3vl_local/action_expert_ablation/bev_only/train.sh --event-balanced
