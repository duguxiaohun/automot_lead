"""LeadMoTPlanningDecoder：LEAD 语义 + prefix-KV MoT 快路。

输入:
    pooled_kv         : list[(K,V)]，长度 = num_layers。
                        每个 K/V 形状 [B 或 1, 8, S, 128]，来自 frozen Qwen prefill。
    bev               : (B, 512, 10, 12)  LEAD LeadTransfuserBackbone 输出
    speed             : (B,) 或 (B, 1)    m/s
    target_point      : (B, 2)            ego frame
    target_point_next : (B, 2)            ego frame

输出:
    pred_route             : (B, 10, 2)  对齐 LEAD data["route"]
    pred_future_waypoints  : (B,  8, 2)  对齐 LEAD data["future_waypoints"]
