"""SFT baseline 训练入口：高速/非高速 + RE/UE 单问直接 token 监督。
...
def _frame_training_pack(
    ...
    *,
    ...
    del event_wrong_prob, event_unknown_prob, clean_event_memory_label, q2_memory_override
...
def _save_adapter(path: pathlib.Path, bundle: Any, args: argparse.Namespace) -> None:
    ...
    payload = {
        "schema_version": 1,
        "route": "sft_baseline_highway_reue_joint",
        ...
        "memory_curriculum": "wrong/UNKNOWN/dropout preserved after binary folding",
...
def _read_resume_config(path: pathlib.Path) -> Dict[str, Any]:
    """读取 checkpoint 自描述配置，并确认它属于当前 sft_baseline 协议。"""

    config_path = path / "sft_baseline_adapter_config.json"
    if not config_path.exists():
        raise FileNotFoundError(f"resume checkpoint missing sft_baseline_adapter_config.json: {path}")
    config = json.loads(config_path.read_text(encoding="utf-8"))
    if config.get("route") != "sft_baseline_highway_reue_joint":
