...
2026-09-26补充：上述固定细分数量若不可行，训练专用 `invalid_capacity.py` 可在同一
INVALID来源内转移次数；来源总额、正例context总额、人工具体题和全局帧cap不变。
原有非空细分组保留至少一次，先最少细分配额偏差，再按原正例动作/不同帧/历史公平费用求解。
重新分配由每台机器的实际池决定，不写死某次日志帧数或缺额；可行后仍由原采样器选帧和推进历史。
无法保留这些约束则报错。该回流只影响binary训练容量失败分支，版本/源码哈希记入训练配置，
不更改共享sampling/build/mapping或验证规则；现有匹配索引可复用，旧run仍用原源码。
