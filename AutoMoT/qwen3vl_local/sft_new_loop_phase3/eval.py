...
def _share_idle_gpu_mask(world_size: int) -> str:
    ...
    path = pathlib.Path(tempfile.gettempdir()) / f"sft_new_loop_phase3_eval_cvd_{master_addr}_{master_port}.txt"
    if rank == 0:
...
def _maybe_set_idle_gpu_mask() -> None:
    ...
    if pinned:
        ...
        if rank == 0:
            print(f"[gpu] using explicit GPU_IDS={pinned}")
        return
...
def setup_distributed() -> Tuple[int, int, int]:
    ...
    if world_size > 1:
        if not torch.cuda.is_available():
            raise RuntimeError("sft_new_loop_phase3 multi-GPU eval requires CUDA.")
    ...
    with path.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            if obj.get("dataset_name") != DATASET_NAME:
                raise ValueError(f"dataset_name mismatch: {obj.get('dataset_name')!r}")
...
def _case_identity_fields(obj: Mapping[str, Any]) -> Tuple[str, str, int, str, str]:
    ...
    return (
        ...
        str(obj.get("context_id", "")),
        str(obj.get("invalid_source") or ""),
    )


def _frame_row_identity(row: FrameRow) -> Tuple[str, str, int, str, str]:
    """返回 ``FrameRow`` 对应的冻结 case 身份。"""

    return (row.scenario, row.route_id, int(row.frame_id), row.context_id, row.invalid_source)


def _read_excluded_case_keys(paths: Sequence[pathlib.Path]) -> Tuple[set, List[str]]:
    """读取旧 eval case JSONL；目录只扫描其顶层 ``cases*.jsonl``。"""
        ...
        with path.open("r", encoding="utf-8") as handle:
            for line_no, line in enumerate(handle, start=1):
                if not line.strip():
                    continue
                try:
                    keys.add(_case_identity_fields(json.loads(line)))
...
def _exclude_prior_cases(
    ...
    *,
    ...
    """从当前 split 排除既有 dev/audit cases。"""
...
def _raw_focus_bin_counts(rows: Sequence[FrameRow]) -> Dict[str, int]:
    """统计 split 中可用的类别、动作签名与 invalid 子类别。"""
...
def _balanced_cases(
    ...
    *,
    ...
    if missing:
        raise ValueError(
            "phase3 eval balance requires every action context plus INVALID; "
    ...
    for key in sorted(groups):
        ...
        if key.endswith("/class/INVALID"):
            # 十个有效 context 各 N，INVALID 为 2N：与构建默认 20% valid 一致。
            target *= 2
            out.extend(
                balanced_invalid_items(
...
def _validate_action_adapter(adapter_dir: pathlib.Path, model_dir: pathlib.Path) -> Dict[str, Any]:
    ...
    if not cfg_path.exists():
        raise FileNotFoundError(f"missing new Phase3 adapter config: {cfg_path}")
    cfg = json.loads(cfg_path.read_text(encoding="utf-8"))
    if cfg.get("route") != "sft_new_loop_phase3_high_level_action":
    ...
    if _resolve_model_path(pathlib.Path(str(saved_model_dir))) != _resolve_model_path(model_dir):
...
def _resolve_adapter_dir(adapter_dir: pathlib.Path) -> Tuple[pathlib.Path, str]:
    ...
    raise FileNotFoundError(
        "cannot resolve new Phase3 adapter. Pass either an adapter dir, or a run dir containing "
        f"final, best_generation, or fallback_generation. Checked: {checked}"
    )
...
def _resolve_history_rgb_mode(
    ...
    """解析 RGB 输入合同；LoRA 以 checkpoint 配置为准。"""
...
def load_eval_bundle(
    ...
    *,
    ...
    if adapter_dir is not None:
        _validate_action_adapter(pathlib.Path(adapter_dir), pathlib.Path(model_dir))
...
def _update_binary_counter(counter: Counter, gt_value: str, pred_value: Optional[str]) -> None:
...
def _copy_case_rgb(case_dir: pathlib.Path, payload: Mapping[str, Any]) -> None:
    ...
    for source_idx, src in zip(
        payload.get("history_rgb_selected_indices") or [], payload.get("history_rgb_paths_used") or []
    ):
...
def _resolve_output_dir(
    base_dir: pathlib.Path, *, timestamp_output: bool, rank: int, world_size: int
) -> pathlib.Path:
...
def evaluate(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    adapter_cfg = (
        _validate_action_adapter(pathlib.Path(args.adapter_dir), pathlib.Path(args.model_dir))
        if args.adapter_dir
        else None
    )
    ...
    if int(args.expected_total_cases) > 0 and len(cases) != int(args.expected_total_cases):
        raise ValueError(
            f"frozen eval case count mismatch: sampled={len(cases)} expected={int(args.expected_total_cases)}"
        )
    ...
    metric_counts: Dict[str, Counter] = {key: Counter() for key in metric_names}
    ...
    with case_path.open("w", encoding="utf-8") as f:
        for local_idx, item in enumerate(local_cases):
            ...
            answer_only_parsed = {key: _parsed_text(answer_only_bool.get(key)) for key in spec.output_keys}
            ...
            for key in ACTION_KEYS:
                ...
                if gt[key] == "YES":
                    ...
                    action_counts[f"{key}/recall_hit"] += int(parsed.get(key) == "YES")
                if parsed.get(key) == "YES":
                    action_counts[f"{key}/pred_yes"] += 1
                    action_counts[f"{key}/precision_hit"] += int(gt[key] == "YES")
            if balance_class == "INVALID":
                for dimension, value in invalid_subgroup_keys(row).items():
            ...
            pattern_counts["pred_invalid_format"] += int(pred_pattern == "INVALID")
            if gt.get(INVALID_KEY) == "YES":
                actions_all_no = all(
                    parsed.get(key) == "NO" for key in spec.output_keys if key != INVALID_KEY
                )
                pattern_counts["invalid_gt_total"] += 1
                pattern_counts["invalid_pred_line_yes"] += int(parsed.get(INVALID_KEY) == "YES")
                pattern_counts["invalid_actions_all_no"] += int(actions_all_no)
                pattern_counts["invalid_joint_ok"] += int(
                    parsed.get(INVALID_KEY) == "YES" and actions_all_no
                )
            payload = {
                "case_index": case_idx,
                ...
                "true_rs": row.true_rs,
                "prompt_road_structure": row.prompt_road_structure,
                "context_id": row.context_id,
                ...
                "context_detail": row.context_detail,
                "goal_ego_xy": list(row.goal_ego_xy),
                ...
                "actual_chat_messages": (
                    [
                        {"role": "system", "content": SYSTEM_PROMPT},
                        {
                            "role": "user",
                            "content": [
                                *[{"type": "image", "source_path": path} for path in used_history_rgb_paths],
            ...
            if not all_ok and bool(args.save_error_rgb):
                _copy_case_rgb(_task_case_dir(error_root, row.context_id, case_idx, row), payload)
            if bool(args.save_all_rgb):
                _copy_case_rgb(_task_case_dir(rgb_root, row.context_id, case_idx, row), payload)
    ...
    for item in gathered:
        ...
        action_counts.update(item.get("action_counts", {}))
        signature_counts.update(item.get("signature_counts", {}))
    ...
    for key in ACTION_KEYS:
        ...
        action_reports[key] = {
            "gt_yes": int(gt_yes),
            "pred_yes": int(pred_yes),
            "recall": float(action_counts.get(f"{key}/recall_hit", 0)) / max(1.0, gt_yes),
            "precision": float(action_counts.get(f"{key}/precision_hit", 0)) / max(1.0, pred_yes),
        }
    signature_reports = {}
    for key, count in sorted(signature_counts.items()):
        ...
        signature_reports[prefix] = {
            "cases": int(count),
            "exact_match_accuracy": float(signature_counts.get(f"{prefix}/exact", 0)) / max(1.0, float(count)),
        }
    invalid_subgroup_accuracy = {}
    ...
    metrics = {
        ...
        "sampling_verification": {
            ...
            "route_diversity": route_diversity_report([item.row.__dict__ for item in cases]),
        ...
        "slice_reports": {
            name.lower(): {
                "cases": int(slice_counts.get(f"{name.lower()}/total", 0)),
        ...
        "invalid_contract": {
            "invalid_gt_total": int(pattern_counts.get("invalid_gt_total", 0)),
            "invalid_line_yes_rate": float(pattern_counts.get("invalid_pred_line_yes", 0)) / invalid_total,
            "invalid_actions_all_no_rate": float(pattern_counts.get("invalid_actions_all_no", 0))
            / invalid_total,
            "invalid_joint_ok_rate": float(pattern_counts.get("invalid_joint_ok", 0)) / invalid_total,
        },
        "invalid_subgroup_accuracy": invalid_subgroup_accuracy,
        "answer_pattern_diagnostics": {key: int(value) for key, value in sorted(pattern_counts.items())},
        "cases_jsonl": (
            str(case_path) if world_size == 1 else [str(item.get("case_path")) for item in gathered]
        ),
    ...
    lines = [
        ...
        f"- invalid_joint_ok_rate: {metrics['invalid_contract']['invalid_joint_ok_rate']:.4f}",
        "",
    ...
    lines.extend(
        ["", "## Action Metrics", "", "| action | gt_yes | pred_yes | recall | precision |", "|---|---:|---:|---:|---:|"]
    )
    for key, report in action_reports.items():
        lines.append(
            f"| {key} | {report['gt_yes']} | {report['pred_yes']} | "
            f"{report['recall']:.4f} | {report['precision']:.4f} |"
        )
    lines.extend(["", "## Action Signature Metrics", "", "| signature | cases | exact |", "|---|---:|---:|"])
    for name, report in signature_reports.items():
        lines.append(f"| {name} | {report['cases']} | {report['exact_match_accuracy']:.4f} |")
    lines.extend(["", "## Invalid Subgroup Metrics", "", "| dimension/value | cases | exact |", "|---|---:|---:|"])
    ...
    lines.append("Wrong examples with RGB are under `error_cases/<CONTEXT>/` when enabled.")
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--timestamp-output", action=argparse.BooleanOptionalAction, default=True)
    p.add_argument("--overwrite", action=argparse.BooleanOptionalAction, default=False)
    p.add_argument("--seed", type=int, default=20260904)
    p.add_argument("--exclude-cases-jsonl", action="append", default=[])
    p.add_argument("--expected-excluded-cases", type=int, default=0)
    p.add_argument("--expected-total-cases", type=int, default=0)
    args = p.parse_args()
