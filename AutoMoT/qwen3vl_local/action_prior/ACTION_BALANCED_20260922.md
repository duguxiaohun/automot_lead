...
> 历史审计：本文件的事件内动作回流、事件硬性等量和action默认上限2已被2026-09-23方案替代。
> 当前是全局动作均衡、小事件权重最多2倍、同源event预算及默认上限8，见[当前说明](GLOBAL_ACTION_BALANCED_20260923.md)。
> 下方数量和命令只解释当时的源码与回放，不能作为当前默认训练计划。

三条 Action 入口新训练默认 `--event-balanced`，加入 `--action-balanced` 切换；仅保留这两种模式。
默认事件采样上限8、动作采样上限2，自动预算可能不同，对照须显式对齐可行预算。
