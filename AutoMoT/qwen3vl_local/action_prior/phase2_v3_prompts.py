# 冻结的新 Phase2 EVENT v3 提示词。来源 commit 6c722de11；不是旧 RS Phase2 包。
# 仅用于恢复已有 v3 adapter 的训练输入，不导入历史训练器、不修改原 adapter 元数据。
...
已经完全进入 ego path；静态事故、路边停车、队列车辆和 ego 前进视差不能伪装成
横向进入。
