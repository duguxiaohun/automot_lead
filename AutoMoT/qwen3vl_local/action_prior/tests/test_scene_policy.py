...
@pytest.mark.parametrize("options,enabled", [
    ([], False), (["--dataset-priors"], False), (["--high-level-action-prior"], False),
    (["--dataset-priors", "--high-level-action-prior"], True),
    (["--dataset-priors", "--high-level-action-prior", "--high-level-action-prior"], True),
    (["--dataset-priors", "--high-level-action-prior", "--prior-noise", "0.1"], False),
    (["--dataset-priors", "--high-level-action-prior", "--no-high-level-action-prior"], False),
])
...
def test_auto_re_reaches_planning_and_action_but_none_retains_context(context, rs):
    ...
    args = parser().parse_args(["--dataset-priors", "--high-level-action-prior", "--high-level-action-prior"])
    ...
    for status in ("selected", "no_action"):
        effective, gate = gate_action(dict(status=status, actions=["STOP"] if status == "selected" else []),
                                   conditions, inputs["high_level_action_contexts"],
                                   inputs["event_balanced_scene_contexts"])
        priors = dict(conditions=conditions, high_level_action_prior=True,
                      high_level_action=effective, high_level_action_gate=gate, event_balanced_scene_contexts=(context,))
        prompt = prompts.prefill_prompt(priors, "nav")
        assert prompts.EVENT_BALANCED_CONTEXT_DESCRIPTIONS[context] in prompt
        assert ("Next action:" in prompt) is (status == "selected")
...
def test_resume_and_eval_keep_saved_scene_condition(tmp_path, enabled, policy):
    ...
    saved = dict(event_balanced_scene_priors=enabled, dataset_priors=True, high_level_action_prior=True)
    if policy:
...
def test_scene_preparation_only_prepares_map_and_never_action_labels(tmp_path, monkeypatch):
    """直接 Python scene 准备入口只准备映射，恢复缺失产物时不重标。"""
    args = parser().parse_args(["--dataset-priors", "--high-level-action-prior"])
    resolve_scene_priors(args)
    calls = []
...
def test_legacy_checkpoint_without_scene_field_stays_disabled():
    ...
    args = SimpleNamespace(dataset_priors=True, high_level_action_prior=True, prior_noise=0)
