#!/usr/bin/env bash
# Phase3 一键评测：base production/audit -> LoRA production/audit -> error audit -> <=30MB 审计包。
#
# 从 AutoMoT/ 目录运行：
#   ADAPTER_DIR=checkpoints/sft_loop_phase3_runs/latest/best_generation bash qwen3vl_local/sft_loop_phase3/eval.sh
# 或：
#   bash qwen3vl_local/sft_loop_phase3/eval.sh checkpoints/sft_loop_phase3_runs/latest/best_generation
...
PHASE_NAME="sft_loop_phase3"
EVAL_PY="qwen3vl_local/sft_loop_phase3/eval.py"
AUDIT_PY="qwen3vl_local/sft_loop_phase3/audit_eval_cases.py"
VISUAL_AUDIT_PY="qwen3vl_local/sft_loop_phase3/visual_audit.py"
ADAPTER_CONFIG_NAME="sft_loop_phase3_adapter_config.json"
...
OUTPUT_ROOT="${OUTPUT_ROOT:-checkpoints/sft_loop_phase3_eval_review/${TIMESTAMP}}"
BUNDLE_MAX_MB="${BUNDLE_MAX_MB:-30}"
ADAPTER_INPUT="${ADAPTER_DIR:-${CKPT_DIR:-${1:-}}}"

if [[ -z "${ADAPTER_INPUT}" ]]; then
  echo "Usage: ADAPTER_DIR=<lora-adapter-or-run-dir> bash qwen3vl_local/sft_loop_phase3/eval.sh" >&2
  exit 2
fi
...
for name in ("sft_loop_phase3_adapter_config.json", "sft_loop_phase2_augment_adapter_config.json", "sft_loop_phase2_adapter_config.json", "sft_loop_phase1_adapter_config.json"):
    path = adapter / name
    if path.is_file():
