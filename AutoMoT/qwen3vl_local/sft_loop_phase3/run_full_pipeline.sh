#!/usr/bin/env bash
# Phase3 event gate 一键流程：build dataset -> base eval -> train -> LoRA eval -> audit samples。
#
# 从 AutoMoT/ 主目录运行：
#   bash qwen3vl_local/sft_loop_phase3/run_full_pipeline.sh
#
# 常用覆盖：
#   GPU_IDS=0,1,2,3 HISTORY_RGB_MODES="4rgb 2rgb_endpoints" bash qwen3vl_local/sft_loop_phase3/run_full_pipeline.sh
#   RUN_TRAIN=0 ADAPTER_DIR=checkpoints/sft_loop_phase3_runs/latest/best_generation bash qwen3vl_local/sft_loop_phase3/run_full_pipeline.sh
...
PIPELINE_ROOT="${PIPELINE_ROOT:-checkpoints/sft_loop_phase3_pipeline/${PIPELINE_TIMESTAMP}}"
...
DATA_OUTPUT_DIR="${DATA_OUTPUT_DIR:-checkpoints/sft_loop_phase3_data}"
...
TRAIN_INVALID_FOCUS_MULTIPLIER="${TRAIN_INVALID_FOCUS_MULTIPLIER:-${INVALID_FOCUS_MULTIPLIER:-1.25}}"
TRAIN_UE1_FOCUS_MULTIPLIER="${TRAIN_UE1_FOCUS_MULTIPLIER:-${UE1_FOCUS_MULTIPLIER:-1.0}}"
TRAIN_UE3_FOCUS_MULTIPLIER="${TRAIN_UE3_FOCUS_MULTIPLIER:-${UE3_FOCUS_MULTIPLIER:-1.0}}"
TRAIN_UE5_FOCUS_MULTIPLIER="${TRAIN_UE5_FOCUS_MULTIPLIER:-${UE5_FOCUS_MULTIPLIER:-1.0}}"
TRAIN_UE6_FOCUS_MULTIPLIER="${TRAIN_UE6_FOCUS_MULTIPLIER:-${UE6_FOCUS_MULTIPLIER:-1.5}}"
...
run_eval() {
  ...
  if [[ "${NPROC}" -gt 1 ]]; then
    ...
    GPU_IDS="${GPU_IDS}" python qwen3vl_local/sft_loop_phase3/eval.py "$@"
  fi
}
...
echo "[pipeline] TRAIN_UE_FOCUS_MULTIPLIERS=${TRAIN_UE1_FOCUS_MULTIPLIER},${TRAIN_UE3_FOCUS_MULTIPLIER},${TRAIN_UE5_FOCUS_MULTIPLIER},${TRAIN_UE6_FOCUS_MULTIPLIER}"
...
if [[ "${RUN_BUILD}" == "1" ]]; then
  ...
  python qwen3vl_local/sft_loop_phase3/build_dataset.py "${BUILD_ARGS[@]}"
fi
...
for HISTORY_RGB_MODE in ${HISTORY_RGB_MODES}; do
  ...
  MODE_TRAIN_OUTPUT_DIR="${TRAIN_OUTPUT_DIR:-checkpoints/sft_loop_phase3_runs/run_event_gate_format_supervised_${HISTORY_RGB_MODE}/${PIPELINE_TIMESTAMP}}"
  ...
  if [[ "${RUN_TRAIN}" == "1" ]]; then
    ...
    UE1_FOCUS_MULTIPLIER="${TRAIN_UE1_FOCUS_MULTIPLIER}" \
    UE3_FOCUS_MULTIPLIER="${TRAIN_UE3_FOCUS_MULTIPLIER}" \
    UE5_FOCUS_MULTIPLIER="${TRAIN_UE5_FOCUS_MULTIPLIER}" \
    UE6_FOCUS_MULTIPLIER="${TRAIN_UE6_FOCUS_MULTIPLIER}" \
    NUM_EPOCHS="${TRAIN_NUM_EPOCHS}" \
    MAX_STEPS="${TRAIN_MAX_STEPS}" \
    bash qwen3vl_local/sft_loop_phase3/train.sh "${TRAIN_MODE}"
...
echo "[done] training runs: checkpoints/sft_loop_phase3_runs/run_event_gate_format_supervised_<mode>/${PIPELINE_TIMESTAMP}"
