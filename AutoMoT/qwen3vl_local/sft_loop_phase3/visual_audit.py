#!/usr/bin/env python3
"""汇总已有逐帧 RGB 审计，并标记不适合视觉监督的 Phase3 RS 风险帧。
...
DEFAULT_COVERAGE_MANIFEST = _THIS.with_name("phase3_rgb_audit_coverage.json")
...
# visual distinction Phase3 learns. They are retained in the index but excluded
...
def _bundled_review_coverage(path: pathlib.Path) -> Dict[str, Any]:
    ...
    payload = json.loads(path.read_text(encoding="utf-8"))
    if payload.get("format") != "sft_loop_phase3_rgb_audit_coverage_v1":
        raise ValueError(f"unsupported Phase3 RGB coverage manifest: {path}")
    coverage: Dict[str, Any] = {}
...
def build_visual_audit_manifest(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    payload = {
        "format": "sft_loop_phase3_visual_audit_manifest",
        "source_review_root": str(review_root),
        "coverage_source": coverage_source,
        "coverage_manifest": str(args.coverage_manifest),
        "coverage_contract": "Every scenario/Town must have at least one completed full-frame RGB review before Phase3 prompt changes.",
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output", default=str(_AUTOMOT_ROOT / "checkpoints/sft_loop_phase3_data/visual_audit_manifest.json"))
    p.add_argument(
        "--coverage-manifest",
...
if __name__ == "__main__":
    ...
    print(f"phase3 RGB audit: routes={result['reviewed_routes']} town_pairs={result['reviewed_scenario_town_pairs']}")
