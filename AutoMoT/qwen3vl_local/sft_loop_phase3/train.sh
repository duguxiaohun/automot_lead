#!/usr/bin/env bash
# sft_loop_phase3 训练 launcher：RS-gated UE1/UE3/UE5/UE6 事件问答 + 可选 torch DDP。
#
# 从 AutoMoT/ 目录运行：
#   GPU_IDS=0 bash qwen3vl_local/sft_loop_phase3/train.sh single
...
INDEX="${INDEX:-checkpoints/sft_loop_phase3_data/frame_index.jsonl}"
...
OUTPUT_DIR_BASE="checkpoints/sft_loop_phase3_runs"
FINAL_RUN_NAME="run_event_gate_format_supervised_${HISTORY_RGB_TAG}"
CHECK_RUN_NAME="check_event_gate_format_supervised_${HISTORY_RGB_TAG}"
...
case "${MODE}" in
  ...
  check)
    ...
    NPROC=1
    EXTRA_ARGS+=(
      # check 模式仍使用构建后索引的各 class 均衡桶，避免只读文件前几行导致漏桶。
...
COMMON_ARGS=(
  ...
  --invalid-focus-multiplier "${INVALID_FOCUS_MULTIPLIER:-1.25}"
  --ue1-focus-multiplier "${UE1_FOCUS_MULTIPLIER:-1.0}"
  --ue3-focus-multiplier "${UE3_FOCUS_MULTIPLIER:-1.0}"
  --ue5-focus-multiplier "${UE5_FOCUS_MULTIPLIER:-1.0}"
  --ue6-focus-multiplier "${UE6_FOCUS_MULTIPLIER:-1.5}"
...
if [[ "${NPROC}" -gt 1 ]]; then
  torchrun --nproc_per_node="${NPROC}" \
    ...
    qwen3vl_local/sft_loop_phase3/train.py "${COMMON_ARGS[@]}"
else
  python qwen3vl_local/sft_loop_phase3/train.py "${COMMON_ARGS[@]}"
fi

echo "[hint] eval base: GPU_IDS=0 python qwen3vl_local/sft_loop_phase3/eval.py"
echo "[hint] eval lora: GPU_IDS=0 python qwen3vl_local/sft_loop_phase3/eval.py --adapter-dir ${OUTPUT_DIR}"
