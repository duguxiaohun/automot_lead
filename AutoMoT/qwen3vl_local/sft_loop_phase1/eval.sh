#!/usr/bin/env bash
# Phase1 一键评测：base production/audit -> LoRA production/audit -> lightweight RGB label audit -> <=30MB 审计包。
#
# 从 AutoMoT/ 目录运行：
#   ADAPTER_DIR=checkpoints/sft_loop_phase1_runs/run_static_obstacle_final_4rgb/final bash qwen3vl_local/sft_loop_phase1/eval.sh
# 或：
#   bash qwen3vl_local/sft_loop_phase1/eval.sh checkpoints/sft_loop_phase1_runs/run_static_obstacle_final_4rgb/final
...
PHASE_NAME="sft_loop_phase1"
EVAL_PY="qwen3vl_local/sft_loop_phase1/eval.py"
...
RUN_LABEL_AUDIT="${RUN_LABEL_AUDIT:-1}"
...
OUTPUT_ROOT="${OUTPUT_ROOT:-checkpoints/sft_loop_phase1_eval_review/${TIMESTAMP}}"
BUNDLE_MAX_MB="${BUNDLE_MAX_MB:-30}"
ADAPTER_INPUT="${ADAPTER_DIR:-${CKPT_DIR:-${1:-}}}"

if [[ -z "${ADAPTER_INPUT}" ]]; then
  echo "Usage: ADAPTER_DIR=<lora-adapter-or-run-dir> bash qwen3vl_local/sft_loop_phase1/eval.sh" >&2
  exit 2
fi
...
path = adapter / "sft_loop_phase1_adapter_config.json"
if path.is_file():
