...
| `U-E2/U-E3/U-E5/U-E8` | OBSTACLE | 需确认对象是否在 ego 路径/短时冲突区，而非远处或旁路。 |
...
- 静态/动态障碍必须侵入或即将侵入 ego 路径，普通跟车/远处车辆不算；
...
- `OBSTACLE` 只看是否有实体占用、进入、阻断或压缩 ego 可行驶走廊；普通跟车、远处车流、
  安全路边停车、事故后残留但路径已打开，都不能自动算障碍。
- `VULNERABLE` 专门看行人/骑行者等未保护参与者是否进入或即将进入 ego 冲突区；远处 sidewalk
  背景人、停车自行车、广告图或反光不算。
- `TRAFFIC_LIGHT_ABNORMAL` 只看控制系统本身是否坏/矛盾/冲突放行/异常熄灭或闪烁。
  `OppositeVehicleRunningRedLight/U-E6` 是车辆违规冲突，通常 `OBSTACLE=YES` 但
...
也就是说，一个 `HIGHWAY:NO` 样本仍然要求模型同时回答 obstacle/vulnerable/traffic-light 三项；
