#!/usr/bin/env bash
# sft_loop_phase1 训练 launcher：四问题 x YES/NO 八桶均衡 + 可选 torch DDP。
#
# 从 AutoMoT/ 目录运行：
...
if [[ "${NPROC}" -gt 1 ]]; then
  torchrun --nproc_per_node="${NPROC}" \
    ...
    qwen3vl_local/sft_loop_phase1/train.py "${COMMON_ARGS[@]}"
else
  python qwen3vl_local/sft_loop_phase1/train.py "${COMMON_ARGS[@]}"
fi

echo "[hint] eval base: GPU_IDS=0 python qwen3vl_local/sft_loop_phase1/eval.py --output-dir checkpoints/sft_loop_phase1_eval/base_zero_shot_prompt"
echo "[hint] eval lora: GPU_IDS=0 python qwen3vl_local/sft_loop_phase1/eval.py --adapter-dir ${OUTPUT_DIR}/final --output-dir ${OUTPUT_DIR}/eval_zero_shot_prompt"
