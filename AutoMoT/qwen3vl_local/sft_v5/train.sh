...
echo "[probe] CHECKPOINT_PROBE=${CHECKPOINT_PROBE:-1} BASE=${CHECKPOINT_PROBE_BASE:-1} TEACHER=${CHECKPOINT_PROBE_WITH_TEACHER:-1} CASES=${CHECKPOINT_PROBE_NUM_CASES:-8} MODE=${CHECKPOINT_PROBE_SAMPLE_MODE:-random} SEQUENCE=${CHECKPOINT_PROBE_SEQUENCE_LENGTH:-8} ARTIFACTS=${CHECKPOINT_PROBE_ARTIFACT_LEVEL:-compact} RADIUS=${CHECKPOINT_PROBE_CONTEXT_RADIUS:-2}"
...
COMMON_ARGS=(
  ...
  --checkpoint-probe-sequence-length "${CHECKPOINT_PROBE_SEQUENCE_LENGTH:-8}"
  --checkpoint-probe-artifact-level "${CHECKPOINT_PROBE_ARTIFACT_LEVEL:-compact}"
