#!/usr/bin/env bash
# SFT v4 launcher: sequence-memory OPD training.
#
# Run from AutoMoT/:
...
VAL_JSONL="${VAL_JSONL:-checkpoints/sft_v4_data/val.jsonl}"
OUTPUT_DIR="${OUTPUT_DIR:-checkpoints/sft_v4_lora}"
...
if [[ "${NPROC}" -gt 1 ]]; then
    torchrun --nproc_per_node="${NPROC}" \
        ...
        qwen3vl_local/sft_v4/train.py "${PY_ARGS[@]}"
else
    python qwen3vl_local/sft_v4/train.py "${PY_ARGS[@]}"
fi

echo "[done] adapter under ${OUTPUT_DIR}"
echo "[hint] eval: GPU_IDS=0 python qwen3vl_local/sft_v4/eval.py --lora-dir ${OUTPUT_DIR}/final --save-root ${OUTPUT_DIR}"
