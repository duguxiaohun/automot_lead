...
Step2 的 `SCENE_CHOICES` 只列 v4 canonical scene：`EnterActorFlowV2` 折叠为
`EnterActorFlow`，`MergerIntoSlowTrafficV2` 折叠为 `MergerIntoSlowTraffic`。这两对
...
`sft_v4_adapter_config.json` 为了审计会保存完整 target module 路径；PEFT 的
