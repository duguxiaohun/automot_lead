...
road_structure_overlay.base_road_structure = base R1/R2/...
...
必须同步写入专用 `road_structure_overlay` / `frame_rs_annotation.overlay`，并兼容性写入
`secondary_road_structures` 与 `frame_rs_annotation.secondary`。例如
`primary_road_structure=R4` 且 `events={R-E4,U-E2}` 时，RS 应为 `R4 + secondary R1/R2`，
且 `road_structure_overlay.active=true`，表示 U-E2 仍属于被路口截断前的基础路段视角。
普通的 `secondary_road_structures` 可能来自候选冲突或不确定性，不能单独用来判断 overlay；
程序化检查必须优先读取 `road_structure_overlay`。
...
- `collector.py::_apply_event_route_postprocess`
  ...
  - 同步把 overlay 的 `base_road_structure` 写入专用 `road_structure_overlay`，
    并兼容性写入 `secondary_road_structures`。
