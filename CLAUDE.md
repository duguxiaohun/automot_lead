...
0.06/0.02 注入错误值/UNKNOWN，EVENT 为 0.10/0.05。每个有效帧都必须运行 Q1；RS
错误只跳过本帧 Q2，下一帧继续分析 RS，直到学生纠正或 delayed repair 执行。RS 连续
错 4 帧后申请修复并每 2 个有效帧 review，EVENT 连续错 3 次后申请修复并每帧 review；
`rs_repair_interval` 只控制脚本兜底，不能用作 Q1 采样间隔。Q1 `ABNORMAL=NO` 不得由
脚本直接把 EVENT 清成 RE。EVENT wrong 扰动优先从本帧 `event_option_map` 其它可见
...
input anomaly rate、error streak、因 RS 错跳过 Q2 的比例，以及 ABNORMAL/UE 的
