...
| `AutoMoT/qwen3vl_local/sft_v5/` 子包 | 按用户同意新增到白名单：SFT v5 是 RS_SLOW / EVENT_FAST 双频 OPSD 路线。数据来自 `AutoMoT/keyframe_filter/collection_output/*_result.json`，训练前跳过 `noScenarios_result.json`、异常时长 route、数据缺失 skip 与缺 XML/RGB/meta/逐帧 annotation 的 route；`review_required=true` 正常参与训练。每帧 meta 抽取 `next_target_points[-1]` 转 ego frame 写成学生可见 `EGO_TO_GOAL_XY`，缺坐标的 frame/旧 index 行直接跳过。Q1 使用 `Scene Description / Critical Object Description / Reasoning on Intent` 三段式 CoT 后只输出 `RS`；Q2 保留同样三段分析后输出 `EVENT`，选项显式标注 `[RE | REGULAR]` / `[UE | UNUSUAL]`，RE 就是 normal、任意 UE 就是 abnormal，不再单问当前是否异常。Q1 memory 只渲染自然语言 `PREVIOUS_RS_HYPOTHESIS + EGO_TO_GOAL_XY`，Q2 才增加 `PREVIOUS_EVENT_HYPOTHESIS`，memory 不写 A-E 或 `RE/U-E*` 标签代码。稳定正确 RS 默认每 4 个 4Hz frame 运行 RS_SLOW，中间快帧复用 RS memory；EVENT_FAST 在每个 RS gate 正确的帧都重新读取当前 RGB，禁止复用前帧 normal/abnormal/EVENT。RS 错误、UNKNOWN 或 recovery 时 RS_SLOW 恢复逐帧，RS 错的当帧跳过 EVENT。EVENT 候选优先用逐帧 `allowed_events`，缺失时才 fallback；所有 `R-E*` 折为一个 `RE`。慢帧 EVENT 精确续接当帧 Q1 KV；快帧没有 Q1 turn，对本帧 RGB fresh prefill。训练用 torchrun 多进程同步 on-policy OPSD，privileged teacher 对 student rollout 的监督 span 做 forward-KL；每帧 loss 立即 backward，optimizer step 前手动 all-reduce LoRA 梯度，禁止包 DDP wrapper。训练不按 `RS:` / `EVENT:` 字段早停。collate 只做 local padding，主进程 all-reduce global `max_T`；默认 `LengthBalancedDistributedSampler`。`train.sh` 支持 `single/ddp/check`；四卡 ddp 默认 `BATCH_PROFILE=max_util`、`PER_DEVICE_BATCH_SIZE=8`、`QWEN_BATCH_SIZE=8`、`PARALLEL_KL_MICROBATCH_SIZE=2`、Q1/Q2 token 上限 1024/1024。batched Q2 rollout 和 parallel KL 都必须按原始 `q1_ids` 精确续接当帧 Q1 KV，禁止 `q1_text` 回环重 tokenize，并保持 mixed-length padding、last-valid logits、OOM 安全微批二分与 M-RoPE delta 形状兼容；详细合同以 `SFT_V5_PLAN.md` 为准。`probe.py` 保存 RGB、student/teacher prompt/output、memory transition、RS_SLOW 是否触发以及 EVENT KV 来源；快帧应显示 `fresh_rgb_prefill`。训练前 base Qwen 体检不传 adapter，teacher/student 输出都从 `Scene Description:` 开始，不复读 MEMORY/choices/REFERENCE。后续改标签协议、prompt、memory、loss、probe 或多卡逻辑时必须同步维护注释与 `SFT_V5_RUN.md` / `SFT_V5_PLAN.md` / `SFT_V5_VISUALIZATION_RECORD.md`。 |
...
`PREVIOUS_RS_HYPOTHESIS + MEMORY_RELIABILITY + EGO_TO_GOAL_XY`，Q2 才额外加入
`PREVIOUS_EVENT_HYPOTHESIS`；memory 必须明确是可能过期或错误的 hypothesis。训练 route
首帧 RS/EVENT 分别以 0.5 概率用 GT，否则为 UNKNOWN；正确 RS memory 默认按
0.06/0.02 注入错误值/UNKNOWN，EVENT 为 0.10/0.05。稳定正确 RS 默认每 4 帧运行
RS_SLOW，中间快帧只复用 RS memory；EVENT_FAST 每个 RS gate 正确的帧都重新
...
`rs_repair_interval` 只控制脚本兜底，与 `rs_slow_interval` 独立。EVENT 的 normal/abnormal
...
82.3 万训练帧。默认 4 帧周期的 RS_SLOW 基础量约 20.6 万帧，加上 memory 扰动/
UNKNOWN/recovery 后预期约 23 万帧；EVENT_FAST 在 RS gate 正确时接近覆盖 82.3
万训练帧。GT UE=15.55% 与 wrong/UNKNOWN memory 异常不能直接相加。
