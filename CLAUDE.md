...
| `AutoMoT/qwen3vl_local/sft_v5/` 检查分类补充约束 | v5 检查按五类维护：训练前 base 能力、训练前 grouped/parallel 等价性、训练中 base/checkpoint/final 自动对比、训练后 adapter 深入可视化、静态 prompt/target 合同快检；大样本 eval 单独负责总体统计。UE 专项必须保留一个连续 UE span 的全部 UE 帧，并按 context radius 补进入前/退出后邻帧；不得被 `num_cases` 从中间截断。RS 专项只需保留变化点前后数帧。 |
