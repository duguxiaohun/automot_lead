...
- `AutoMoT/keyframe_filter/`（递归整目录白名单，包含现有文件、子目录文件和未来新增文件）
...
- **禁止** `git add keyframes_all_scenarios.json` 或 `AutoMoT/lead_data/keyframes_all_scenarios.json`；
  `AutoMoT/keyframe_filter/` 下的同名文件属于该目录递归白名单，可随目录精确 add
