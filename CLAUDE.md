...
| `AutoMoT/tools/SFT_V2_PLAN.md` / `SFT_V2_RUN.md` / `build_sft_dataset_v2_teacher.py` / `sft_v2_loss_scale_plugin.py` / `sft_v2_train.sh` / `check_loss_mask_v2.py` / `inspect_teacher_outputs.py` | SFT v2 升级：teacher 用冻结 base Qwen + PRIVILEGED prompt 离线产 ANALYSIS 真值，student 全段都算 loss（ANALYSIS 段 0.3 / STATUS,SUBGOAL event_name 1.0 / 字面与空白 0）。`build_sft_dataset_v2_teacher.py` 加载 pending jsonl 跑 teacher 推理填 ANALYSIS，支持中断续跑 + 多卡分片；`sft_v2_loss_scale_plugin.py` 注册策略名 `sft_v2_analysis_supervised`；`sft_v2_train.sh` 与 v1 同套 GPU/MASTER_PORT 自动选址逻辑；`check_loss_mask_v2.py` 用于 v2 token 级静态 sanity；`inspect_teacher_outputs.py` 用于 teacher 产物可视化抽检（默认只读 jsonl，`--live` 可现场重跑） |
