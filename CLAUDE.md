...
| `AutoMoT/qwen3vl_local/sft_v4/` 子包（含 `__init__.py` / `SFT_V4_PLAN.md` / `SFT_V4_RUN.md` / `prompts.py` / `build_dataset.py` / `train.py` / `train.sh` / `eval.py` / `probe.py` / `check_loss_mask.py` / `test_memory_update.py` / `test_kv_reuse.py` / `test_gt_leak_filter.py`） | 按用户同意新增到白名单：SFT v4 是 sequence-memory OPD 的 off-policy 演进路线；当前代码先完成 v4 命名、路径、adapter config 与文档自洽。`SFT_V4_PLAN.md` 固化 actor-learner 方案：collector 异步采集 rollout 并写 replay shard，不进入 DDP；learner 才进入 DDP，同步读取 replay 做梯度 allreduce 并周期发布 adapter snapshot。四张 H20 默认锁定 GPU0/GPU1 各 1 个 learner DDP rank，GPU2/GPU3 各 3 个 collector；batch 仍为每 rank 1，collector 不与 learner 混部，后续可按 replay 水位/MPS 观测调到每 collector 卡 2-4 个进程。后续新增 `replay.py` / `collect.py` / `learn.py` / `launch_offpolicy.sh` 时需同步加入 AGENTS/CLAUDE 白名单与 git add 示例。 |
