...
| `AutoMoT/qwen3vl_local/sft_v5/` checkpoint probe 补充约束 | 正式 launcher 默认 `SAVE_STEPS=40`（用户实测约 80 optimizer steps/day，约半天一版）。默认在 step 0 生成 `probes/base/`，每个 `checkpoint-*` 和 `final/` 保存后对固定 seed 与相同 `random` 规则选出的一段连续 8 帧 validation 片段生成 LoRA probe；`--num-cases` 是总帧预算，`--sequence-length` 是每段目标长度。公开选帧模式只保留 `random` / `rs_transition` / `ue_transition`：RS 专项保留同一次变化的前帧/新 RS 首帧/后帧，UE 专项保留同一 UE span 的进入前 RE、UE 入口/内部/末帧和退出后 RE；专项找不到变化时不用无关帧 fallback 凑数。默认 `--artifact-level review` 按 `scenarios/<scenario>__<route>/frame_<id>/` 保存连续帧，每帧分别写输入、学生/教师输出、teacher target、场景 GT、memory、evaluation 和 `prediction_vs_ground_truth.json`；该对照文件必须并列当前 RS/ABNORMAL/EVENT 真值、原始 scenario/route 审计真值、学生 Q1/Q2 解析结构、默认与按双标签动态解析的结构化真值、文本 teacher target 和逐项正确性，并按字段给出 `expected/predicted/correct`；未运行 student 时预测与正确性必须为 null。`compact` 只写同 schema 的顶层 `results.json`；`full` 在 review 文件之上增加 `selection_plan.json`、逐帧 `case_record.json`、RGB/prompt/memory/timeline。各版本摘要汇总到 `probes/comparison.json`。自动 probe 必须复用 rank0 当前训练 bundle：base student 与 privileged teacher 临时 `disable_adapter()`，checkpoint/final student 保持 LoRA 开启；禁止另起子进程或加载第二份 Qwen。其它 rank 在 probe 前后 barrier，完成后恢复 train 模式并清理 CUDA cache；probe 失败写 `error.txt` 后继续训练。probe 的 256/192 token 上限只控制可视化耗时，不改变训练 1024/1024。 |
| `AutoMoT/qwen3vl_local/sft_v5/` eval/probe 指标补充约束 | 大样本 `eval.py` 与小样本 `probe.py` 必须共用 `metrics.py`，统一统计 RS/UE 边界、Q1/Q2 precision/recall/F1、假阳性/假阴性、端到端 EVENT 与 route macro 指标；另外必须用相邻帧 GT/预测状态分别统计 RS 变化、RE->UE 进入和 UE->RE 退出的 TP/FP/TN/FN/invalid、precision/recall/F1 与 false-positive-rate。小样本三种 artifact mode 都把变化报告内嵌在 `results.json.transition_report`，full 模式再另写 `transition_report.json`；eval 可用 `--transition-jsonl` 只落盘变化和 FP/FN 轻量记录。结果中保存每个指标的中文含义和“越高越好/越低越好/仅诊断”方向。eval 默认按帧流式累计，只有显式 `--output-jsonl` 才落盘全量逐帧 prompt/output，不能为统计把全量输入输出常驻内存。 |
| `AutoMoT/qwen3vl_local/sft_v5/` 检查分类补充约束 | v5 检查按五类维护：训练前 base 能力、训练前 grouped/parallel 等价性、训练中 base/checkpoint/final 自动对比、训练后 adapter 深入可视化、静态 prompt/target 合同快检；大样本 eval 单独负责总体统计。compact `results.json` 只减少文件数量，不能减少人工审计字段：每帧必须保存 RGB 路径、实际 student/teacher messages、完整 student/base-teacher CoT 输出、脚本化 teacher target、RS/EVENT 场景 GT、memory 和变化检测结果；base 与 LoRA probe 使用同一 schema，唯一差别是 student 是否开启 adapter。UE 专项必须保留一个连续 UE span 的全部 UE 帧，并按 context radius 补进入前/退出后邻帧；不得被 `num_cases` 从中间截断。RS 专项只需保留变化点前后数帧。 |
| `AutoMoT/qwen3vl_local/sft_v5/` 显存生命周期与 teacher probe 补充约束 | 纯 batched rollout 只返回文本/token ids，不得为每个完成样本物化或返回 final KV；Q2 state 构造后立即释放旧 Q1/Q2 KV。每个 loss backward 后释放计算图引用，optimizer step 使用 `zero_grad(set_to_none=True)`；正常/异常退出统一销毁 process group 并执行一次 GC/CUDA cache 清理，但训练 step 内不得频繁 `empty_cache()`。teacher Q2 能力指标只在 teacher 自身 Q1 RS 正确时触发，必须续接 teacher 自己的 Q1 KV、RS 和 ABNORMAL；训练用 `q2_teacher_training_prompt.txt` 与自主 teacher 的 `q2_teacher_model_prompt.txt` 分开保存，默认 `q2_teacher_prompt.txt` 必须与 `q2_teacher_output.txt` 配对。 |
| `AutoMoT/qwen3vl_local/sft_v5/` 注释与文档分工补充约束 | 每个 Python 模块需用中文 docstring 说明用法和入口，所有 class/function（含 CLI、嵌套 helper 和魔术方法）都需有中文 docstring；非显然的 padding、KV、loss 分母、DDP collective 和显存生命周期逻辑需注释设计原因，不写逐行复述。后续改标签协议、prompt、memory、loss、probe 或 DDP 训练逻辑时必须同步维护相邻注释。`SFT_V5_RUN.md` 保持为精简的可执行命令手册；设计合同放在 `SFT_V5_PLAN.md`，完整 probe 产物和人工检查项放在 `SFT_V5_VISUALIZATION_RECORD.md`，不在三份文档间重复铺开。 |
