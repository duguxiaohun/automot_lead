...
| `qwen3vl_local/sft_base/` | SFT v5 的直接监督基线。复用 v5 的 collection_output 数据构建、异常 route 剔除、4 帧 RGB history、`EGO_TO_GOAL_XY`、RS/EVENT 候选池和串行 memory 状态；Q2 候选顺序使用 v5 seed namespace，但本路线不再使用 A/B/C 字母，学生直接输出语义 token。训练不做 OPSD、不采 student rollout、不跑 privileged teacher、不输出 CoT；Q1 target 只有 `RS: <token>`，`ABNORMAL` 已删除，Q2 target 只有 `EVENT: <token>`，UE/RE 指标从 Q2 EVENT 折叠。训练为 teacher-forced weighted CE，memory 由 GT answer 推进，但 prompt memory 默认强扰动：RS wrong/UNKNOWN=0.30/0.40、EVENT wrong/UNKNOWN=0.35/0.35、memory dropout=0.15，`route_state` 将 wrong/UNKNOWN/keep 连续保持 3-5 个真实帧，转折邻域 repeat=4/window=3；验证 loss 使用同款 prompt-memory 扰动且 seed 固定不随 rank 漂移，避免 clean memory 低估 val loss。route 首帧固定 UNKNOWN/UNKNOWN 并直接返回；非首帧 dropout 是独立第一层，命中就隐藏 RS/EVENT 先验并跳过其它扰动；RS unknown 时 EVENT 仍独立扰动，RS wrong 时 EVENT 在 UNKNOWN 与新 RS 自洽错误候选之间分流；R3 对抗只保留 GT=R3 -> memory=R1，不再做 R1 -> R3 反向灌入。Q1 prompt 不显示 `BELIEVED_EVENT`，只用 RS memory 和 goal；Q1 后 RS hypothesis 真正改变时旧 EVENT 立即失效为 UNKNOWN，训练侧随后为 Q2 按当前 RS 池单独重采 EVENT memory，且 keep 分支使用进入本帧前的干净 EVENT memory（上一帧 GT），避免 Q2 EVENT 边际塌成 UNKNOWN 或泄漏本帧答案；GT EVENT 在候选表内的单/多候选 Q2 都参与训练：单候选 RE 低权重、多候选 RE 保留为硬负样本、UE 保持高权重，UE 子类默认按逆频率 repeat 放大长尾。eval 可省略 `--adapter-dir` 直接跑 base 零样本基线；LoRA eval 会校验 adapter config。eval Q2 候选按学生 RS 的静态候选全集生成；逐帧 `allowed_events` 只用于 GT 解析和审计，不参与 prompt 候选构造，避免候选长度泄漏当帧异常事实；集合与 dataset 候选相同时复用 dataset 顺序。GT EVENT 不在 dataset 自己候选表中的帧与训练 Q2 skip 对齐，从 EVENT 准确率、UE-vs-RE、EVENT 混淆和 EVENT change 指标分母中剔除，并用 `dataset_candidate_mismatch_{rate,ue_rate,re_rate}` 单独报告；GT EVENT 在 dataset 候选中但不在学生 RS 候选中仍算模型错误。`EGO_TO_GOAL_XY` 每帧刷新，eval 支持 `--ablate-goal` 隐藏导航目标；eval 默认首帧 UNKNOWN 冷启动、Q1 错也继续问 Q2，并输出 RS/EVENT 混淆矩阵、R3 shortcut 监控、UE-vs-RE P/R/F1、单/多候选 × RE/UE 四格 EVENT 指标、按候选数量分层的 `q2_candidate_count_report`、`ue_fp_on_multi_candidate_re_rate`、`single_candidate_invalid_rate`、相邻帧 RS change / RE->UE / UE->RE 的 TP/FP/FN/TN/F1/invalid、GT 稳定帧假转折率、窗口左边界已在目标值比例、转折方向 sparse confusion 和 EVENT 不可达率；`*_hit_offset_avg` 在 summary 中按 frame 显示，不再误格式化成百分比。`audit_eval_candidate_drift.py` 可量化 eval 候选过滤相对训练候选的偏差并拆分 set mismatch / scoreable unreachable / dataset mismatch examples。默认 `LORA_VISION_SCOPE=last4`、rank=32、alpha=64、vision_lr_scale=0.2，并启用视觉 fuse guard；仍可用 `off/merger/all` 做对照。运行见 `qwen3vl_local/sft_base/SFT_BASE_RUN.md`。 |
| `qwen3vl_local/sft_base/` RS×UE 静态表补充约束 | `EVENT_CANDIDATES_BY_RS` 按严格方案 A 维护：复用 build_dataset 的异常/缺失/失败 route 过滤后，只保留 `count >= 20` 且 `rs_frame_rate >= 0.1%` 的 RS×UE 组合；当前 prompt 候选数为 R1=6、R2=4、R3=1、R4=5、R5=5，R3 仍保持纯 RE。`audit_rs_event_cooccurrence.py` 必须报告 missing、low-rate 和 spurious 静态组合；`audit_eval_candidate_drift.py --expect-mismatch-combinations` 只断言 GT static mismatch 的组合类型落在已知低频拒绝集合内，不按全量 163 帧硬套 train/val split。eval 指标需同时保留 `q2_candidate_count_report` 与 `q2_rs_candidate_count_report`，避免不同 RS 候选难度混在同一个 EVENT 准确率里。 |
