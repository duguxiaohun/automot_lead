...
| `qwen3vl_local/sft_v5/` | SFT v5 RS_SLOW / EVENT_FAST 双频 OPSD 路线：数据来自 `keyframe_filter/collection_output/*_result.json`，每帧 meta 抽取 `next_target_points[-1]` 转 ego frame 写成学生可见 `EGO_TO_GOAL_XY`，缺坐标的 frame/旧 index 行直接跳过。Q1 保留 `Scene Description / Critical Object Description / Reasoning on Intent` 三段式 CoT 后只输出 `RS`；Q2 保留同样三段分析后输出 `EVENT`，候选显式标注 `[RE | REGULAR]` / `[UE | UNUSUAL]`，RE 表示 normal、任意 UE 表示 abnormal，不再单独询问当前是否异常。Q1 memory 只渲染自然语言 `PREVIOUS_RS_HYPOTHESIS + EGO_TO_GOAL_XY`，Q2 才增加 `PREVIOUS_EVENT_HYPOTHESIS`，不写 A-E 或 `RE/U-E*` 标签代码。稳定正确 RS 默认每 4 个 4Hz frame 运行一次 RS_SLOW，中间快帧复用 RS memory；EVENT_FAST 在每个 RS gate 正确的帧都重新读取当前 RGB，禁止复用前帧 normal/abnormal/EVENT。RS 错误、UNKNOWN 或 recovery 时 RS_SLOW 恢复逐帧，RS 错的当帧跳过 EVENT。EVENT 候选优先用逐帧 `allowed_events`，缺失时才 fallback；所有 `R-E*` 折成一个 `RE`，原始细分只作审计。慢帧 EVENT 作为第二轮 user turn 精确续接当帧 Q1 KV；快帧没有 Q1 turn，对本帧 RGB fresh prefill。训练使用 torchrun 多进程同步 on-policy OPSD：student rollout 后由 privileged teacher 对相同监督 span 做 forward-KL，每帧 loss 立即 backward，optimizer step 前手动 all-reduce LoRA 梯度；禁止 DDP wrapper，训练不按 `RS:` / `EVENT:` 字段早停。DataLoader 采用 local padding + global max_T，同卡支持 batched rollout 与独立 parallel-KL 微批；batched Q2 rollout 与 KL 都按原始 `q1_ids` 精确续接当前 Q1 KV，禁止文本回环重 tokenize。四卡 ddp 默认 8 route rollout、2 route KL 微批、Q1/Q2 token 上限 1024/1024。每帧 loss 按全局有效 frame 等权。`probe.py` 保存 RGB、student/teacher prompt/output、memory transition、RS_SLOW 触发原因和 EVENT KV 来源，快帧必须标为 `fresh_rgb_prefill`。运行、指标与可视化详见 `SFT_V5_RUN.md` / `SFT_V5_PLAN.md` / `SFT_V5_VISUALIZATION_RECORD.md`。 |
...
| `qwen3vl_local/sft_v5/` memory curriculum 补充约束 | 旧 `BELIEVED_*` 名称已废弃：Q1 使用 `PREVIOUS_RS_HYPOTHESIS + MEMORY_RELIABILITY + EGO_TO_GOAL_XY`，Q2 才额外加入 `PREVIOUS_EVENT_HYPOTHESIS`；memory 是可能过期或错误的 hypothesis。训练 route 首帧 RS/EVENT 各以 0.5 概率使用 GT，否则为 UNKNOWN；正确 RS memory 默认按 0.06/0.02 注入错误值/UNKNOWN，EVENT 为 0.10/0.05。稳定正确 RS 默认 `rs_slow_interval=4`，快帧不产生 RS rollout/loss，但每个 RS gate 正确的帧都必须产生 EVENT_FAST rollout/loss。RS 错误只跳过本帧 EVENT，下一帧恢复逐帧 RS 分析，直到学生自行纠正或 delayed repair 执行。RS 连错 4 帧后申请修复并每 2 个有效帧 review，EVENT 连错 3 次后申请修复并每帧 review；`rs_repair_interval` 只控制兜底，与 `rs_slow_interval` 独立。EVENT 的 RE/UE family 直接由当前 EVENT 选项推导，不存在独立 ABNORMAL 输出或可复用状态；Q1/Q2 最终高权重 span 只监督单个选项字符。训练/TB 记录错误 memory 复制、自主恢复、注入、强制修复、input anomaly rate、error streak、RS_SLOW 触发/复用率、因 RS 错跳过 EVENT 的比例及由 EVENT 折叠出的 UE/RE TP/FP/TN/FN/P/R/F1；eval/probe 同步输出 memory 依赖与门控指标。42 个有效 collection 场景的 success 上限为 7241 route/914466 帧，默认 10% validation 后约 82.3 万训练帧；精确值以远端 build summary 为准。 |
| `qwen3vl_local/sft_v5/` memory 数据量审计 | 42 个有效 `collection_output` 场景（排除 `noScenarios`）共有 7241 条 success route、914466 帧；原始 GT EVENT 中 RE=772286（84.45%）、UE=142180（15.55%）。默认 10% route-level validation 后约 82.3 万训练帧，最终值以远端 `checkpoints/sft_v5_data/summary.json` 为准。GT UE 与人为 wrong/UNKNOWN memory 是两种不同“异常”，不能直接相加。默认 4 帧周期的 RS_SLOW 基础量约 20.6 万帧（25%），加上 8% memory 扰动/UNKNOWN/recovery 后预期触发率约 28%、约 23 万帧；EVENT_FAST 在 RS gate 正确时接近覆盖 82.3 万训练帧。按平均约 126 帧/route 模拟，RS memory anomaly 约为 8.4% 当帧自纠下限到 27.5% 持续复制上界；EVENT 为实际 EVENT_FAST 帧的约 15.3% 到 31.7%。 |
