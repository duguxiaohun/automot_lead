...
| `qwen3vl_local/sft_new_loop_phase2/run_leadmot_qwen_ab.sh` / `compare_leadmot_qwen_ab.py` | Phase2 v3 seed 20260810 的自动下游收口：不改 prompt、不打开 unseen；默认 LeadMoT train/val JSONL 同时缺失时从 `lead_data` 自动构建 no-subgoal 索引（含异常 route 剔除和 route-level split，单边缺失拒绝）；分别训练 base-Qwen/LoRA-Qwen 的 LeadMoT decoder，对同一 validation case 配对并按 route cluster bootstrap。只有 route/waypoint ADE/FDE 四项 95% CI 上界均小于 0 才允许进入 CARLA。 |
