...
| `qwen3vl_local/sft_v4/` | SFT v4 off-policy actor-learner 路线：collector 异步用 adapter snapshot 采集 sequence-memory rollout 并写 replay shard；learner 才进入 DDP，同步读取 replay 做梯度 allreduce 并周期发布新 adapter snapshot。默认四卡 H20 部署锁定 GPU0/GPU1 各 1 个 learner DDP rank、GPU2/GPU3 各 3 个 collector；Phase A 初始正确率 0.5、Phase B 噪声率 0.15、learner world_size=2。当前代码已完成 v4 命名/路径/adapter config 自洽，具体实现分阶段计划见 `SFT_V4_PLAN.md` |
