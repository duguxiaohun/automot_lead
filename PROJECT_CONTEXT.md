...
| `qwen3vl_local/sft_v5/` | SFT v5 RS/EVENT 两问串行 OPSD 路线：数据来自 `keyframe_filter/collection_output/*_result.json`，每帧 meta 抽取 `next_target_points[-1]` 转 ego frame 写成学生可见 `EGO_TO_GOAL_XY`，缺该坐标的 frame/旧 index 行会被跳过，不能继续显示 UNKNOWN；Q1 使用精简 `Scene Description / Critical Object Description / Reasoning on Intent` 三段式 CoT 后输出 `RS / ABNORMAL`；system prompt 简短提醒关注交通灯/标志、周围车辆/行人/障碍物、车道线/道路结构和影响自车决策的关键因素；Q1 memory 只渲染自然语言 `BELIEVED_RS + EGO_TO_GOAL_XY`，不带 `BELIEVED_EVENT`，Q2 才渲染自然语言 `BELIEVED_EVENT`，memory 文本不写 A-E 选项字母或 `RE/U-E*` 标签代码，Q2 在 RS 正确后按逐帧 `allowed_events` 判断 `RE` vs `U-E*`；所有 `R-E*` 只折成一个 `RE` 训练类，原始 `event_code` / `regular_event_codes` 作审计和 RE 细分解释。训练使用 torchrun 多进程同步 on-policy 边采样边训练：每张卡都先让当前 student rollout Q1/Q2 token，Q2 作为 Q1 assistant 输出后的第二轮 user turn 复用 Q1 KV cache，再用 privileged teacher logits 对同一批 token 的监督 span 做 forward-KL；每帧 loss 立刻 backward，只累计 LoRA 梯度，并在 optimizer step 前手动 all-reduce；训练阶段禁止按 `ABNORMAL:` 或 `EVENT:` 字段结构早停，避免缩短 CoT 并改变 OPSD 监督分布；不能包 `DistributedDataParallel(model)` wrapper，因为动态 Q2 分支会造成 rank 间 forward 次数不一致并触发 NCCL watchdog。当前不是 v4 那种 collector/learner 异步 replay 分卡架构。DataLoader collate 只做 local padding，主训练进程 all-reduce global `max_T` 后补齐，padding frame 不读图、不进 Qwen、不产 loss；多卡默认使用 `LengthBalancedDistributedSampler` / `SAMPLER_MODE=length_balanced`，在每个 rank route 数一致的前提下按 route frame 数均衡分片，减少长 route rank 拖住其它 rank；`SAMPLER_MODE=distributed` 可切回 PyTorch 原生 `DistributedSampler` 做对照；`train.sh` 支持 `single/ddp/check` 与自动 GPU 选址、防覆盖 run 目录，四卡 `ddp` 默认 H20 max_util 8 路口径：`BATCH_PROFILE=max_util`、`PER_DEVICE_BATCH_SIZE=8`、`QWEN_BATCH_SIZE=8`、`MAX_NEW_TOKENS_Q1=1024`、`MAX_NEW_TOKENS_Q2=1024`、`PROGRESS_FRAMES=20`，启动时打印 `[batch]` 配置，第一条 `[batch-start]` 应显示 `routes=8 / qwen_batch=8`；`BATCH_PROFILE=balanced` 退回 6 路，`BATCH_PROFILE=debug` 退回 4 路；`single/check` 默认仍保守 `1/1`。rank0 会输出 batch/frame/sync 心跳，默认 `LOGGING_STEPS=1`，可用 `PROGRESS_FRAMES` 和 `HEARTBEAT_SECONDS` 调整日志密度；阶段 1 batched Qwen 通过 `QWEN_BATCH_SIZE` 启用，批量化同一 timestep 多 route 的 Q1/Q2 student rollout，需要配合 `PER_DEVICE_BATCH_SIZE>1`；batched Q1 必须按 `attention_mask` 取最后真实 token logits、repetition penalty 不得包含 padding token，CUDA OOM 不允许静默 fallback，开大前用 `test_batched_qwen_smoke.py --check-parallel-kl` 做 single-vs-batch Q1/Q2 续接、训练 logits 和 parallel-KL-vs-逐帧-KL 总 loss / case loss / parts 对照；parallel KL 的显存峰值主要来自 `batch x rollout_len x vocab` student/teacher logits，开 8 路前必须小 run 验证。每帧 loss 按全局有效 frame 数归一化，手动 all-reduce 后保持 frame 等权。`probe.py` 仿 v3 可视化产物：route/frame 层级、4 帧 RGB 副本、system/user/messages 分离视图、student prompt/output、teacher privileged prompt、脚本化 teacher target、可选 base Qwen teacher output、memory_before/after、flags、timeline 和 manifest；`--with-teacher-model` 用于训练前 base Qwen OPSD 能力体检，该体检必须不传 `--adapter-dir`、不加载任何 LoRA，只测试普通 Qwen 的 teacher/student 能力；teacher model output 应和 student 一样从 `Scene Description:` 开始输出分析与 `RS/EVENT`，不能复读 MEMORY、choices 或 REFERENCE；训练后 adapter 学生可视化只需 `--with-model --adapter-dir ...`。v5 代码已补中文函数说明和关键逻辑块注释，后续改标签协议、prompt、memory、loss、probe 或 DDP 训练逻辑时需同步维护注释。可视化速查见 `SFT_V5_VISUALIZATION_RECORD.md`。运行见 `SFT_V5_RUN.md` |
| `qwen3vl_local/sft_v5/` batched Qwen 补充约束 | Q1/Q2 student rollout 允许 mixed-length padded batch；padded KV 只用于 no-grad 采样，不写回 memory。默认保持 `QWEN_BATCH_SIZE=8`，但有 autograd graph 的 parallel KL 使用独立 `PARALLEL_KL_MICROBATCH_SIZE=2`，即 8 路 rollout 后按 2+2+2+2 teacher/student scoring 并逐微批 backward。Q2 KL 必须按精确 `q1_ids` 续接 Q1 KV 后追加 Q2 user turn，禁止文本回环重 tokenize。KL forward OOM 只允许在尚未 backward 时二分当前微批，不降低 token 上限、不重新 rollout；backward OOM 或普通异常必须中止，避免部分梯度后 fallback 重复累计。`test_batched_qwen_smoke.py --check-parallel-kl` 验证真实模型等价性，`test_parallel_kl_microbatch.py` 验证微批/OOM 二分梯度等价性。显存峰值按 `KL microbatch x context length` 审计，TB 记录 `parallel_kl/{microbatches_per_chunk,frames_per_microbatch,oom_splits}`。相关代码必须保留中文注释解释 padded rollout、精确 Q2 KV 续接、KL OOM 安全二分和 TensorBoard 分母口径。 |
| `qwen3vl_local/sft_v5/` TensorBoard 补充约束 | 除 `train/loss_frame` 外，必须记录 `train/loss/{q1_analysis,q1_rs,q1_abnormal,q2_analysis,q2_event}`，其中 Q1 分项按有效 frame 平均，Q2 分项按实际进入 Q2 的 frame 平均；同时记录 `memory/{allocated,reserved,max_allocated,max_reserved}_gb`，长期显存风险以活跃引用 `allocated` 为主，不能只凭 `nvidia-smi` 或 allocator `reserved` 高水位判断泄漏。 |
...
| `qwen3vl_local/sft_v5/` checkpoint probe 补充约束 | 正式 launcher 默认 `SAVE_STEPS=40`，按用户实测约 80 optimizer steps/day 即约半天一版。每个 run 在 step 0 生成 `probes/base/`，每个 `checkpoint-*` 和 `final/` 保存后对固定 seed 与相同 `random` 规则选出的一段连续 8 帧 validation 片段生成 LoRA probe；`--num-cases` 是总帧预算，`--sequence-length` 是每段目标长度。公开选帧模式只保留 `random` / `rs_transition` / `ue_transition`：RS 专项保留同一次变化的前帧/新 RS 首帧/后帧，UE 专项保留同一 UE span 的进入前 RE、UE 入口/内部/末帧和退出后 RE；专项找不到变化时不用无关帧 fallback 凑数。默认 `--artifact-level review` 按 `scenarios/<scenario>__<route>/frame_<id>/` 保存连续帧；每帧分开保存输入、学生/教师输出、teacher target、场景 GT、memory、evaluation 和 `prediction_vs_ground_truth.json`。对照文件直接包含当前 RS/ABNORMAL/EVENT 真值、原始 scenario/route 审计真值、学生解析结构、默认/双标签动态结构化真值、文本 teacher target 与逐项正确性，并按字段给出 `expected/predicted/correct`；未运行 student 时预测与正确性为 null。`compact` 只写同 schema 的顶层 `results.json`；`full` 再增加 RGB、legacy prompt/TXT、timeline 和 manifest。各版本摘要由 run 级 `probes/comparison.json` 聚合。自动 probe 复用 rank0 当前训练 bundle，不另起进程、不加载第二份 Qwen：base student 与 privileged teacher 临时 `disable_adapter()`，checkpoint/final student 保持 LoRA；其它 rank 在 probe 前后 barrier。完成后恢复 train 模式并清 CUDA cache；probe 失败写 `error.txt` 后继续训练。probe 默认 256/192 token 只控制可视化耗时，不改变训练 1024/1024。 |
| `qwen3vl_local/sft_v5/` eval/probe 指标补充约束 | `eval.py` 与 `probe.py` 共用 `metrics.py`，统一统计 RS/UE 边界、Q1/Q2 precision/recall/F1、假阳性/假阴性、端到端 EVENT 与 route macro 指标；另外用相邻帧 GT/预测状态统计 RS 变化、RE->UE 进入和 UE->RE 退出的 TP/FP/TN/FN/invalid、precision/recall/F1 与 false-positive-rate。小样本三种 artifact mode 都把变化报告内嵌在 `results.json.transition_report`，full 模式再另写 `transition_report.json`；eval 可用 `--transition-jsonl` 只落盘变化和 FP/FN 轻量记录。输出同时保存中文定义和指标方向。大样本 eval 默认流式累计，只有显式 `--output-jsonl` 才落盘全量逐帧输入输出，不能为统计把全量 prompt/output 常驻内存。 |
| `qwen3vl_local/sft_v5/` 显存生命周期与 teacher probe 补充约束 | 纯 batched rollout 只返回文本/token ids，不物化逐样本 final KV；Q2 state 构造后立即释放旧 Q1/Q2 KV。loss backward 后释放计算图，optimizer step 使用 `zero_grad(set_to_none=True)`；正常/异常退出统一销毁 process group 并做一次 GC/CUDA cache 清理，训练 step 内不频繁 `empty_cache()`。teacher Q2 仅在 teacher 自身 Q1 RS 正确时触发，并续接 teacher 自己的 Q1 KV/解析 memory；训练 privileged prompt 与 teacher 自主 prompt 分文件保存。 |
| `qwen3vl_local/sft_v5/` 注释与文档分工补充约束 | 每个 Python 模块用中文 docstring 说明用法和入口，所有 class/function（含 CLI、嵌套 helper 和魔术方法）保留中文 docstring；padding、KV、loss 分母、DDP collective 和显存生命周期等非显然逻辑需注释设计原因。compact `results.json` 只减少文件数量，不能减少人工审计字段：每帧保存 RGB 路径、实际 student/teacher messages、完整 student/base-teacher CoT 输出、脚本化 teacher target、RS/EVENT 场景 GT、memory 和变化检测结果；base 与 LoRA probe 使用同一 schema。UE 专项必须保留一个连续 UE span 的全部 UE 帧，并按 context radius 补进入前/退出后邻帧，不得被 `num_cases` 从中间截断；RS 专项只保留变化点前后数帧。`SFT_V5_RUN.md` 只作精简命令手册，设计合同归 `SFT_V5_PLAN.md`，完整 probe 产物和人工检查项归 `SFT_V5_VISUALIZATION_RECORD.md`。 |
