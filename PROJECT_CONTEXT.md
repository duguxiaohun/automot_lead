...
| `qwen3vl_local/sft_v5/` | SFT v5 RS/EVENT 两问串行 OPSD 路线：数据来自 `keyframe_filter/collection_output/*_result.json`，Q1 判断 RS + 是否异常，Q2 在 RS 正确后按逐帧 `allowed_events` 判断 `RE` vs `U-E*`；所有 `R-E*` 只折成一个 `RE` 训练类，原始 `event_code` / `regular_event_codes` 作审计和 RE 细分解释。训练使用 true torch DDP，同步 on-policy 边采样边训练：每张卡都先让当前 student rollout Q1/Q2 token，再用 privileged teacher logits 对同一批 token 做 forward-KL 并 DDP 同步梯度；当前不是 v4 那种 collector/learner 异步 replay 分卡架构。DataLoader collate 只做 local padding，主训练进程 all-reduce global `max_T` 后补齐，padding frame 不读图、不进 Qwen、不产 loss；`train.sh` 支持 `single/ddp/check` 与自动 GPU 选址、防覆盖 run 目录。`probe.py` 仿 v3 可视化产物：route/frame 层级、4 帧 RGB 副本、student prompt/output、teacher privileged prompt、脚本化 teacher target、可选 base Qwen teacher output、memory_before/after、flags、timeline 和 manifest；`--with-teacher-model` 用于训练前 base Qwen OPSD 能力体检，该体检必须不传 `--adapter-dir`、不加载任何 LoRA，只测试普通 Qwen 的 teacher/student 能力；训练后 adapter 学生可视化只需 `--with-model --adapter-dir ...`。v5 代码已补中文函数说明和关键逻辑块注释，后续改标签协议、prompt、memory、loss、probe 或 DDP 训练逻辑时需同步维护注释。可视化速查见 `SFT_V5_VISUALIZATION_RECORD.md`。运行见 `SFT_V5_RUN.md` |
