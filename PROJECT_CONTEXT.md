...
| `qwen3vl_local/sft_new_loop_phase2/run_leadmot_qwen_ab.sh` / `compare_leadmot_qwen_ab.py` | Phase2 v3 seed 20260810 的自动下游收口：不改 prompt、不打开 unseen；分别训练 base-Qwen/LoRA-Qwen 的 LeadMoT decoder，对同一 validation case 配对并按 route cluster bootstrap。只有 route/waypoint ADE/FDE 四项 95% CI 上界均小于 0 才允许进入 CARLA。 |
