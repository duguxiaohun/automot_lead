...
| `qwen3vl_local/sft_base/` RS×EVENT 静态表补充约束 | `EVENT_CANDIDATES_BY_RS` 按严格方案 A 维护 UE 组合：复用 build_dataset 的异常/缺失/失败 route 过滤后，只保留 `count >= 20` 且 `rs_frame_rate >= 0.1%` 的 RS×UE 组合；regular 子类直接保留各 RS 的 R-E 候选。当前 prompt 候选数为 R1=7、R2=5、R3=3、R4=5、R5=5，R3 不开放 UE 但不再是单候选。`audit_rs_event_cooccurrence.py` 必须报告 RS×UE、RS×R-E regular 分布、多 regular 标签比例、missing、low-rate 和 spurious 静态组合；`audit_eval_candidate_drift.py --expect-mismatch-combinations` 只断言 GT static mismatch 的组合类型落在已知低频拒绝集合内，不按全量 163 帧硬套 train/val split。 |
