...
| `qwen3vl_local/sft_v5/` checkpoint probe 补充约束 | 正式 launcher 默认 `SAVE_STEPS=40`，按用户实测约 80 optimizer steps/day 即约半天一版。每个 run 在 step 0 生成 `probes/base/`，每个 `checkpoint-*` 和 `final/` 保存后对固定 8 个、相同 seed 与相同 `random` 规则选出的 validation case 生成 LoRA probe。公开选帧模式只保留 `random` / `rs_transition` / `ue_transition`：RS 专项保留同一次变化的前帧/新 RS 首帧/后帧，UE 专项保留同一 UE span 的进入前 RE、UE 入口/内部/末帧和退出后 RE；专项找不到变化时不用无关帧 fallback 凑数。`selection_plan.json` 记录选择原因，每帧 `case_record.json` 保存完整输入输出。各版本写 `summary.json`，run 级 `probes/comparison.json` 聚合 base/checkpoint/final。自动 probe 复用 rank0 当前训练 bundle，不另起进程、不加载第二份 Qwen：base student 与 privileged teacher 临时 `disable_adapter()`，checkpoint/final student 保持 LoRA；其它 rank 在 probe 前后 barrier。完成后恢复 train 模式并清 CUDA cache；probe 失败写 `error.txt` 后继续训练。probe 默认 256/192 token 只控制可视化耗时，不改变训练 1024/1024。 |
| `qwen3vl_local/sft_v5/` eval/probe 指标补充约束 | `eval.py` 与 `probe.py` 共用 `metrics.py`，统一统计 RS/UE 边界、Q1/Q2 precision/recall/F1、假阳性/假阴性、端到端 EVENT 与 route macro 指标；另外用相邻帧 GT/预测状态统计 RS 变化、RE->UE 进入和 UE->RE 退出的 TP/FP/TN/FN/invalid、precision/recall/F1 与 false-positive-rate。小样本写 `transition_report.json`；eval 可用 `--transition-jsonl` 只落盘变化和 FP/FN 轻量记录。输出同时保存中文定义和指标方向。大样本 eval 默认流式累计，只有显式 `--output-jsonl` 才落盘全量逐帧输入输出，不能为统计把全量 prompt/output 常驻内存。 |
...
| `qwen3vl_local/sft_v5/` 注释与文档分工补充约束 | 每个 Python 模块用中文 docstring 说明用法和入口，所有 class/function（含 CLI、嵌套 helper 和魔术方法）保留中文 docstring；padding、KV、loss 分母、DDP collective 和显存生命周期等非显然逻辑需注释设计原因。`SFT_V5_RUN.md` 只作精简命令手册，设计合同归 `SFT_V5_PLAN.md`，完整 probe 产物和人工检查项归 `SFT_V5_VISUALIZATION_RECORD.md`。 |
