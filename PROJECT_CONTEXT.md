...
| `qwen3vl_local/sft_v5/` checkpoint probe 补充约束 | launcher 默认 `SAVE_STEPS=40`，step 0、checkpoint、final 都用固定 seed 测同一条完整 validation route ID，从首帧运行到末帧；`--num-routes` 控制 random ID 数，`--num-cases` 只用于 RS/UE 专项，专项默认边界前后 8 帧且 UE span 不截断。完整 ID 首帧初始化 student/reference，之后 student RS/EVENT 仅由自身 Q1/Q2 输出推进，reference 只比较不纠错，逐帧只刷新导航坐标。`results.json.memory_recovery_report` 统计变化后 student 首次自行对齐的延迟。默认 review 每帧只写 `input_rgb_*.jpg`、`input.json`、`output.json`、`memory.json`；output 含 student/teacher raw+parsed、teacher target、场景 GT 和正确性，memory 含两问 student 转换与 comparison-only reference。`compact` 只写顶层 results，`full` 额外保留 legacy 文件。自动 probe 复用 rank0 bundle，base/teacher 临时关闭 adapter，checkpoint/final student 使用 LoRA；其它 rank barrier，结束恢复 train 并清 cache。 |
...
| `qwen3vl_local/sft_v5/` memory curriculum 补充约束 | 旧 `BELIEVED_*` 名称已废弃：Q1 使用 `PREVIOUS_RS_HYPOTHESIS + MEMORY_RELIABILITY + EGO_TO_GOAL_XY`，Q2 才额外加入 `PREVIOUS_EVENT_HYPOTHESIS`；memory 是可能过期或错误的 hypothesis。训练 route 首帧 RS/EVENT 各以 0.5 概率使用 GT，否则为 UNKNOWN；正确 RS memory 默认按 0.06/0.02 注入错误值/UNKNOWN，EVENT 为 0.10/0.05。每个有效帧都运行 Q1；RS 错误只跳过本帧 Q2，下一帧继续分析 RS，直到学生自行纠正或 delayed repair 执行。RS 连错 4 帧后申请修复并每 2 个有效帧 review，EVENT 连错 3 次后申请修复并每帧 review；`rs_repair_interval` 只控制脚本兜底，不能作为 Q1 采样间隔。Q1 `ABNORMAL=NO` 不得脚本清空 EVENT；Q1/Q2 最终高权重 span 只监督单个选项字符。训练/TB 记录错误 memory 复制、自主恢复、注入、强制修复、input anomaly rate、error streak、因 RS 错跳过 Q2 的比例及 ABNORMAL/UE 的 TP/FP/TN/FN/P/R/F1；eval/probe 同步输出 memory 依赖与 Q2 门控指标。42 个有效 collection 场景的 success 上限为 7241 route/914466 帧，默认 10% validation 后约 82.3 万训练帧；精确值以远端 build summary 为准。 |
| `qwen3vl_local/sft_v5/` memory 数据量审计 | 42 个有效 `collection_output` 场景（排除 `noScenarios`）共有 7241 条 success route、914466 帧；原始 GT EVENT 中 RE=772286（84.45%）、UE=142180（15.55%）。默认 10% route-level validation 后约 82.3 万训练帧，最终值以远端 `checkpoints/sft_v5_data/summary.json` 为准。GT UE 与人为 wrong/UNKNOWN memory 是两种不同“异常”，不能直接相加。按平均约 126 帧/route 模拟，默认新阈值的 RS memory anomaly 约为 8.4% 当帧自纠下限到 27.5% 持续复制上界；EVENT 为实际 Q2 帧的约 15.3% 到 31.7%。 |
